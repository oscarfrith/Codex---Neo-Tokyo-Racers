...
local function buildModals()
	...
	-- Route guide destinations (opened by clicking the minimap). Rows come from Config.UI.RouteGuide.Destinations.
	local destinations = RouteGuide.Destinations()
	local routes = modalShell("Routes", "ROUTE GUIDE", 520, 150 + #destinations * 56)
	for index, destination in ipairs(destinations) do
		local row = button(routes, "Route_" .. destination.Id, string.upper(destination.DisplayName), UDim2.fromOffset(440, 46), UDim2.fromOffset(40, 64 + (index - 1) * 56), C("Panel"), C("Outline"))
		row.Activated:Connect(function()
			RouteGuide.SetDestinationById(destination.Id, "Player")
			closeModal()
			showToast("ROUTE SET: " .. string.upper(destination.DisplayName), true)
		end)
	end
	local clearRoute = button(routes, "ClearRoute", "CLEAR ROUTE", UDim2.fromOffset(212, 46), UDim2.new(0, 40, 1, -66), C("Panel"), C("Danger"))
	clearRoute.Activated:Connect(function()
		RouteGuide.Clear("Player")
		closeModal()
		showToast("ROUTE CLEARED", true)
	end)
	local closeRoutes = button(routes, "Close", "CLOSE", UDim2.fromOffset(212, 46), UDim2.new(1, -252, 1, -66), C("PanelBlue"), C("Telemetry"))
	closeRoutes.Activated:Connect(closeModal)
end
...
local function buildMainHud()
	...
	routeButton.Activated:Connect(function() openModal("Routes") end)
