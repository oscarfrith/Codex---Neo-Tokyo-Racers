-- NTR Persistence Phase 23 Garage Interior Service Canonical Repair
-- Dual-mode Studio Command Bar script.
--
-- Edit mode:
--   Replaces GarageInteriorService_Active with one canonical Phase 21-23 source.
--   This avoids fragile text-patching after partial installs.
--
...
local TAG = "[NTR Persistence Phase 23 Garage Interior Service Canonical Repair]"
...
if RunService:IsRunning() then
	...
	assert(type(initial) == "table" and initial.Profile ~= nil, "Garage GetInitial failed before canonical visit smoke.")
	...
	info("Expected: canonical Phase 21-23 garage interior service is active and same-server visit actions work.")
	return
end
...
local displayRuntime = garageServices:FindFirstChild("GarageDisplayRuntime")
assert(displayRuntime and displayRuntime:IsA("ModuleScript"), "Expected Phase 22 GarageDisplayRuntime before canonical Phase 23 repair.")

local interiorService = garageServices:FindFirstChild("GarageInteriorService_Active")
assert(interiorService and interiorService:IsA("Script"), "Expected GarageInteriorService_Active before canonical Phase 23 repair.")

local canonicalSource = [=[-- NTR Persistence Phase 21-23 Garage Interior Service Canonical Source
...
interiorService.Source = canonicalSource
interiorService.Disabled = false
interiorService:SetAttribute("PersistencePhase23CanonicalService", true)
interiorService:SetAttribute("PersistencePhase23SameServerGarageVisits", true)

local finalSource = interiorService.Source
assert(string.find(finalSource, "PersistencePhase23CanonicalService", 1, true), "Canonical service marker missing.")
assert(string.find(finalSource, "VisitGarage", 1, true), "VisitGarage action missing.")
assert(string.find(finalSource, "SetAccessMode", 1, true), "SetAccessMode action missing.")
assert(string.find(finalSource, "findPlayerInServerByUserId", 1, true), "Safe same-server owner lookup missing.")
assert(not string.find(finalSource, "Players:GetPlayerByUserId", 1, true), "Unsafe Players:GetPlayerByUserId lookup is still present.")

info("PASS: replaced GarageInteriorService_Active with canonical Phase 21-23 source.")
