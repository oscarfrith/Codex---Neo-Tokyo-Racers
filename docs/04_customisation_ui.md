...
The Phase 1 screenshot review found structural layout issues rather than isolated colour tweaks. Phase 2A-D established the visual system, Phase 2F fixed physical card clearance, and Phase 2G-I completed live boost telemetry. Phase 3C returns the minimap to an unrotated four-ImageLabel north-up canvas for reliable clipping and original artwork fidelity. Phase 3D is generated as `scripts/roblox_ui_freeroam_pc_phase3d_image_only_map_markers.lua`; it makes the centred player marker a transparent image-only overlay and adds a second transparent image-only north arrow at the bottom-right with editable asset, size, and inset.
