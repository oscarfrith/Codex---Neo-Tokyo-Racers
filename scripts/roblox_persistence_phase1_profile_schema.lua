-- Neo Tokyo Racers - Persistence Phase 1
-- Profile schema foundation for owned vehicle/module instances and garage data.
--
-- Safe foundation phase:
-- - Does not patch active garage, UI, driving, VFX, or DataStore scripts.
-- - Creates a shared PlayerProfileSchema ModuleScript and editable persistence config.
-- - Prepares DataStore-safe encode/decode helpers for future ProfileService work.

local ReplicatedStorage = game:GetService("ReplicatedStorage")

local PHASE = "Persistence Phase 1"
...
setDefaultAttribute(persistenceConfig, "SchemaVersion", 1)
setDefaultAttribute(persistenceConfig, "DataStoreName", "NTR_PlayerProfiles_v1")
setDefaultAttribute(persistenceConfig, "StartingGarageCapacity", 2)
setDefaultAttribute(persistenceConfig, "AutosaveSeconds", 90)
setDefaultAttribute(persistenceConfig, "SaveDebounceSeconds", 8)
setDefaultAttribute(persistenceConfig, "SameServerGarageVisitsOnly", true)
setDefaultAttribute(persistenceConfig, "UseInstancedInteriorGarages", true)
setDefaultAttribute(persistenceConfig, "DefaultGarageAccessMode", "Private")
setDefaultAttribute(persistenceConfig, "EditNote", "Persistence Phase 1 schema/config only. No DataStore or live garage actions are switched yet.")

ensureModule(dataModules, "PlayerProfileSchema", schemaSource)

local ok, schemaOrError = pcall(function()
	return require(dataModules:WaitForChild("PlayerProfileSchema"))
end)
if not ok then
	error("PlayerProfileSchema require failed: " .. tostring(schemaOrError))
end

local schema = schemaOrError
local profile = schema.DefaultProfile(140000)
local safe, encodedOrError = schema.AssertDataStoreSafe(profile)
if not safe then
	error("Default profile is not DataStore-safe: " .. tostring(encodedOrError))
end

info("Installed ReplicatedStorage.NeoTokyoRacers.Shared.Modules.Data.PlayerProfileSchema")
info("Installed ReplicatedStorage.NeoTokyoRacers.Shared.Config.Persistence_EditAttributes")
info("Default schema summary: vehicles=" .. tostring(schema.Summarize(profile).VehicleCount) .. ", capacity=" .. tostring(schema.Summarize(profile).GarageCapacity))
info("No active gameplay scripts were changed.")
