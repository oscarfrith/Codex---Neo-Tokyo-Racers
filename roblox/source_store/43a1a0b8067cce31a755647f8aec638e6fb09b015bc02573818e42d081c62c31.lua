...
-- Route guide destinations (opened by tapping the minimap). Rows come from Config.UI.RouteGuide.Destinations.
local function showRoutes()
	local destinations=RouteGuide.Destinations()
	openModal("ROUTE GUIDE",520,150+#destinations*56)
	for index,destination in ipairs(destinations) do
		local row=button(modalBody,"Route_"..destination.Id,string.upper(destination.DisplayName),UDim2.new(1,-40,0,46),UDim2.fromOffset(20,(index-1)*56),CYAN)
		row.Activated:Connect(function() RouteGuide.SetDestinationById(destination.Id,"Player"); closeModal(); showToast("ROUTE SET: "..string.upper(destination.DisplayName),true) end)
	end
	local clearRoute=button(modalBody,"ClearRoute","CLEAR ROUTE",UDim2.fromOffset(200,44),UDim2.new(0,20,1,-50),DANGER)
	clearRoute.Activated:Connect(function() RouteGuide.Clear("Player"); closeModal(); showToast("ROUTE CLEARED",true) end)
	local done=button(modalBody,"Done","CLOSE",UDim2.fromOffset(200,44),UDim2.new(1,-220,1,-50),CYAN); buttonGradient(done); done.Activated:Connect(closeModal)
end
...
routeButton.Activated:Connect(showRoutes)
...
local function majorMenu() return player:GetAttribute("GarageSessionActive")==true or playerGui:GetAttribute("OwnedGarageManagementOpen")==true end
