-- Neo Tokyo Racers - PC Free-Roam UI Phase 4A Dealership Teleport
...
-- Preserves Phase 3D and connects the existing confirmation modal to an
-- isolated server-authoritative dealership teleport service. It does not patch
-- the register-limited bootstrap or large garage controller.

local PHASE = "NTR PC Free-Roam UI Phase 4A Dealership Teleport"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE4A_DEALERSHIP_TELEPORT"
...
local runtimeRoot = ensure(configRoot, "Folder", "Runtime")
local teleportConfig = ensure(runtimeRoot, "Folder", "FreeRoamHudTeleport")

value(hudConfig, "BoolValue", "Enabled", true)
value(hudConfig, "StringValue", "InstalledPhase", MARKER).Value = MARKER
value(teleportConfig, "NumberValue", "CooldownSeconds", 2)
value(teleportConfig, "NumberValue", "HeightOffset", 4)
value(teleportConfig, "NumberValue", "ForwardOffset", 0)
value(teleportConfig, "NumberValue", "UnseatSettleSeconds", 0.08)
value(teleportConfig, "NumberValue", "VehicleDespawnDelaySeconds", 0.12)
value(teleportConfig, "NumberValue", "CharacterUnfreezeDelaySeconds", 0.18)
...
value(layout, "NumberValue", "MapPlayerIconSize", 22)
value(layout, "NumberValue", "MapNorthArrowSize", 28)
value(layout, "NumberValue", "MapNorthArrowMargin", 10)
...
value(defaults, "StringValue", "Minimap", "NORTH").Value = "NORTH"
value(defaults, "BoolValue", "MapFlipX", false)
value(defaults, "BoolValue", "MapFlipZ", false)
value(defaults, "BoolValue", "MapPlayerIconRotates", true)
...
value(assets, "StringValue", "MapPlayerIcon", "")
value(assets, "StringValue", "MapNorthArrow", "")
...
local remotes = ensure(shared, "Folder", "Remotes")
local uiRemotes = ensure(remotes, "Folder", "UI")
local teleportInvoke = ensure(uiRemotes, "RemoteFunction", "FreeRoamHudTeleportInvoke")

local serverRoot = ServerScriptService:WaitForChild("NeoTokyoRacers")
local services = serverRoot:WaitForChild("Services")
local uiServices = ensure(services, "Folder", "UI")
local teleportService = ensure(uiServices, "Script", "FreeRoamHudTeleportService_Active")
teleportService.Source = [==[
...
teleportService.Disabled = false
teleportService:SetAttribute("InstalledBy", MARKER)
teleportService:SetAttribute("InstalledAt", os.date("!%Y-%m-%dT%H:%M:%SZ"))
...
if controller then
	...
	local knownPhase3C = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE3C_NORTH_UP_2D_MINIMAP", 1, true)
	local knownPhase3D = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE3D_IMAGE_ONLY_MAP_MARKERS", 1, true)
	local knownPhase4A = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE4A_DEALERSHIP_TELEPORT", 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H or knownPhase2I or knownPhase3A or knownPhase3B or knownPhase3C or knownPhase3D or knownPhase4A, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
info("Installed the canonical isolated Phase 4A dealership teleport.")
info("YES now calls an isolated server-authoritative service with cooldown, race guard, vehicle despawn, and safe character teleport.")
info("Phase 4A preserves Phase 3D and does not patch the bootstrap, garage controller, driving controller, racing services, or VFX.")
info("Next: restart Play and verify NO, YES on foot, YES while driving, cooldown, target placement, and race rejection.")
