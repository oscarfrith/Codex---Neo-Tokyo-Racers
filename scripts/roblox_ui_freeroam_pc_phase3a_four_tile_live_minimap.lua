-- Neo Tokyo Racers - PC Free-Roam UI Phase 3A Four-Tile Live Minimap
...
-- Phase 2I and installs a live centred minimap assembled from four 1024x1024
-- image tiles. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 3A Four-Tile Live Minimap"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE3A_FOUR_TILE_LIVE_MINIMAP"
...
if controller then
	...
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H or knownPhase2I or knownPhase3A, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE3A_FOUR_TILE_LIVE_MINIMAP
...
local function updateRuntime(dt)
	...
	if mapSubject and mapSubject:IsA("BasePart") then
		...
		local targetRotation = mode == "ROTATE" and (-heading + rotationOffset) or rotationOffset
		local radians = math.rad(targetRotation)
		local localPoint = Vector2.new(dx * uiPerStud, dz * uiPerStud)
		local rotatedPoint = Vector2.new(localPoint.X * math.cos(radians) - localPoint.Y * math.sin(radians), localPoint.X * math.sin(radians) + localPoint.Y * math.cos(radians))
		local targetPosition = Vector2.new(mapSize * 0.5, mapSize * 0.5) - rotatedPoint
		...
		mapCanvas.Rotation = displayedMapRotation
		playerArrow.Rotation = mode == "ROTATE" and 0 or heading
	end
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase3A", 3000, function(dt)
	updateRuntime(dt)
end)
]=]
...
info("Installed the canonical isolated Phase 3A four-tile live minimap.")
info("Phase 3A startup repair included: removed the stale pre-minimap arrow.ZIndex reference.")
info("Map calibration: 207 pixels = 2850 studs, 2048px total, world origin 0,0,0 at image centre.")
info("Set the four DesktopFreeRoamHud.Assets.MapTile... image ids before final alignment testing.")
info("Phase 3A preserves Phase 2I and does not patch driving, bootstrap, server, VFX, or mobile UI.")
info("Next: restart Play, add tile ids, and verify X/Z orientation, scale, translation, and rotation mode.")
