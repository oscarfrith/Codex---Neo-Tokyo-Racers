-- Neo Tokyo Racers - PC Free-Roam UI Phase 2I Borderless Boost Icon
...
-- Phase 2H but removes every visible icon-container surface so only the
-- configured boost image is rendered. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 2I Borderless Boost Icon"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2I_BORDERLESS_BOOST_ICON"
...
if controller then
	...
	local knownPhase2I = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H or knownPhase2I, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
info("Installed the canonical isolated Phase 2I borderless boost-icon repair.")
info("Only the configured boost image is visible; the transparent positioning container has no surface, stroke, or glow.")
info("Phase 2I preserves live boost telemetry and does not patch driving, bootstrap, or mobile UI.")
info("Next: restart Play and verify the image-only icon plus live drain/recharge.")
