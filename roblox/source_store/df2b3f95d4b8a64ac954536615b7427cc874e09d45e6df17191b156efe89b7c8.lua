...
local SharedUI = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("RacingUIComponents"))
local RouteGuide = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("RouteGuide"))
...
local DisplayNames = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("Vehicles"):WaitForChild("VehicleDisplayNames"))
local VehicleCards = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("GarageComponents"))
local config = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud")
local racingPerformanceConfig = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("Racing"):WaitForChild("PresentationPerformance")
local colours = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Colours")
local layoutConfig = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Layout")
local assetConfig = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Assets")
local defaults = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Defaults")
local typography = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Typography")
local effects = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Effects")
...
local function isMajorMenuOpen()
	for _, screen in ipairs(playerGui:GetChildren()) do
		if screen:IsA("ScreenGui") and screen ~= gui and screen.Enabled then
			if player:GetAttribute("GarageSessionActive") == true then return true end
...
local function fireUiEvent(name)
	local event = game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"):FindFirstChild(name)
...
local function loadingAction(action, payload)
	...
	warn("[Desktop HUD] Loading transition " .. tostring(action) .. " failed: " .. tostring(result))
	return nil
end
...
local function makeSegmented(parent, y, titleText, options, selected, onPick)
	...
	for option, item in pairs(items) do
		item.MouseButton1Click:Connect(function()
			for other, otherItem in pairs(items) do
				local active = other == option
				otherItem.BackgroundColor3 = active and C("PanelBlue") or C("Panel")
				setAccent(otherItem, active and C("Telemetry") or C("Outline"))
			end
			onPick(option)
		end)
	end
...
local function buildModals()
	...
	-- Saved on the server (profile.Settings.PassengerAccess) through the activities remote.
	local passengerLabels = { Friends = "FRIENDS", Anyone = "ANYONE", Nobody = "NOBODY" }
	-- Reflects the saved value (the modal is built before the profile loads) and server rejections.
	function syncPassengers()
		local current = passengerLabels[player:GetAttribute("PassengerAccess")] or "FRIENDS"
		for _, option in ipairs({ "FRIENDS", "ANYONE", "NOBODY" }) do
			local item = left:FindFirstChild("PASSENGERS" .. option)
			if item then
				item.BackgroundColor3 = option == current and C("PanelBlue") or C("Panel")
				setAccent(item, option == current and C("Telemetry") or C("Outline"))
			end
		end
	end
	makeSegmented(left, 352, "PASSENGERS", { "FRIENDS", "ANYONE", "NOBODY" }, passengerLabels[player:GetAttribute("PassengerAccess")] or "FRIENDS", function(option)
		local remotes = game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):FindFirstChild("Activities")
		local remote = remotes and remotes:FindFirstChild("ActivityInvoke")
		if not remote then return end
		task.spawn(function()
			local ok, reply = pcall(function() return remote:InvokeServer("SetPassengerAccess", { Access = string.sub(option, 1, 1) .. string.lower(string.sub(option, 2)) }) end)
			if not (ok and type(reply) == "table" and reply.Ok) then showToast("PASSENGER SETTING NOT SAVED", false) end
			syncPassengers()
		end)
	end)
	player:GetAttributeChangedSignal("PassengerAccess"):Connect(function() syncPassengers() end)
	syncPassengers()
	...
	-- Session-only choice (settings are not persisted yet); both HUD owners read the MinimapMode attribute.
	local minimapChoice = player:GetAttribute("MinimapMode")
	makeSegmented(right, 283, "MINIMAP", { "ROTATE", "NORTH UP" }, minimapChoice == "NORTH UP" and "NORTH UP" or "ROTATE", function(option)
	...
	-- Route destinations moved to the full map legend (FullMapUI).
end
...
local function buildMainHud()
	...
	-- Driver Rank strip (Street Life): sits just above the cash chip and follows it. Attributes come from ProgressionService.
	rankStrip = new("Frame", { Name = "RankStrip", BackgroundColor3 = C("PanelDeep"), BackgroundTransparency = 0.28, BorderSizePixel = 0, Position = UDim2.new(0, 0, 0, -26), Size = UDim2.new(1, 0, 0, 22), Visible = false, ZIndex = money.ZIndex }, money)
	corner(rankStrip, 6)
	local rankLabel = label(rankStrip, "Rank", "RANK 1", UDim2.new(0.5, 0, 1, -4), UDim2.fromOffset(8, 0), T("Caption", 11), C("Telemetry"))
	local xpLabel = label(rankStrip, "Xp", "", UDim2.new(0.5, -8, 1, -4), UDim2.new(0.5, 0, 0, 0), 10, C("Muted"), Enum.TextXAlignment.Right)
	local xpTrack = new("Frame", { Name = "XpTrack", BackgroundColor3 = C("PanelSoft"), BorderSizePixel = 0, Position = UDim2.new(0, 6, 1, -5), Size = UDim2.new(1, -12, 0, 3), ZIndex = rankStrip.ZIndex + 1 }, rankStrip)
	local xpFill = new("Frame", { Name = "XpFill", BackgroundColor3 = C("Telemetry"), BorderSizePixel = 0, Size = UDim2.fromScale(0, 1), ZIndex = xpTrack.ZIndex + 1 }, xpTrack)
	local function renderRank()
		local rank, into, need = player:GetAttribute("Rank"), tonumber(player:GetAttribute("XpIntoRank")) or 0, tonumber(player:GetAttribute("XpForNext")) or 0
		rankStrip.Visible = rank ~= nil
		rankLabel.Text = "RANK " .. tostring(rank or 1)
		xpLabel.Text = need > 0 and (tostring(into) .. " / " .. tostring(need) .. " XP") or "MAX"
		xpFill.Size = UDim2.fromScale(need > 0 and math.clamp(into / need, 0, 1) or 1, 1)
	end
	for _, attribute in ipairs({ "Rank", "XpIntoRank", "XpForNext" }) do player:GetAttributeChangedSignal(attribute):Connect(renderRank) end
	renderRank()

	minimap = new("CanvasGroup", { Name = "Minimap", BackgroundColor3 = C("PanelDeep"), BackgroundTransparency = 0.28, BorderSizePixel = 0, Position = UDim2.fromOffset(0, cashHeight + 8), Size = UDim2.fromOffset(mapSize, mapSize), ClipsDescendants = true, ZIndex = 8 }, leftCluster)
	corner(minimap, 9)
	-- The CanvasGroup clips rotated content to the box and rounded corners; the rotator turns the map around its centre (the player).
	mapRotator = new("Frame", { Name = "MapRotator", AnchorPoint = Vector2.new(0.5, 0.5), BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.fromScale(0.5, 0.5), Size = UDim2.fromScale(1, 1), ZIndex = 9 }, minimap)
	...
	mapPlayerMarkers = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("FreeRoamMapPlayerMarkers")).new({
		Container = minimap,
		Config = game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("FreeRoamMapPlayerMarkers"),
		ZIndex = 13,
	})
	-- Upright place/job/waypoint icons (MapMarkers), above the edge fades and below the player arrow.
	mapIcons = require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("MapIconLayer")).new({
		Container = minimap, Surface = "Minimap", ZIndex = 15, Font = FONT,
		Theme = { PanelDeep = C("PanelDeep"), Panel = C("Panel"), Outline = C("Outline"), OutlineSoft = C("OutlineSoft"), Telemetry = C("Telemetry"), ElectricBlue = C("ElectricBlue"), HighSpeed = C("HighSpeed"), Danger = C("Danger"), Text = C("Text"), Muted = C("Muted") },
	})
	routeRenderer = RouteGuide.newMapRenderer({
		Canvas = mapCanvas, Container = minimap, ZIndex = 10, ChipZIndex = 16, Font = FONT,
		Colours = { Line = C("Telemetry"), Activity = C("HighSpeed"), Outline = C("PanelDeep"), ChipBackground = C("PanelDeep"), ChipText = C("Text") },
	})
	-- Clicking the minimap opens the full map (FullMapUI, started by ClientBase).
	local routeButton = new("TextButton", { Name = "RouteButton", Text = "", AutoButtonColor = false, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.fromScale(1, 1), ZIndex = 17 }, minimap)
	routeButton.Activated:Connect(function()
		local module = game:GetService("ReplicatedStorage").Modules.Game.UI:FindFirstChild("FullMapUI")
		local loaded, fullMap = pcall(function() return module and require(module) end)
		if not (loaded and type(fullMap) == "table" and fullMap.Open()) then showToast("MAP NOT AVAILABLE", false) end
	end)
	RouteGuide.Arrived:Connect(function(destination) showToast("ARRIVED: " .. destination.Label, true) end)
	...
	local onboardingControls=game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"):WaitForChild("OpenDrivingControlsFromOnboarding")
...
local function ensureGui()
	local existing = playerGui:FindFirstChild("DesktopFreeRoamHud")
	if existing then existing:Destroy() end
	gui = new("ScreenGui", { Name = "DesktopFreeRoamHud", IgnoreGuiInset = true, ResetOnSpawn = false, DisplayOrder = 85, ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, playerGui)
...
local function updateRuntime(dt)
	...
	-- FullMapUI owns the screen (and RouteGuide.Update) while the full map is open.
	if player:GetAttribute("FullMapOpen") == true then gui.Enabled=false; closeChoiceList(); return end
	gui.Enabled=true
	...
	local ownedGarageManagementOpen = playerGui:GetAttribute("OwnedGarageManagementOpen") == true
	...
	if not ownedGarageInside and not (racingPresentationActive and readValue(racingPerformanceConfig, "PauseFreeRoamMapDuringRace", true) == true) then
		...
		if mapSubject and mapSubject:IsA("BasePart") then
			...
			local rotation = mapPlayerMarkers.ResolveRotation(mapPlayerMarkers.Config, player)
			local rotationOffset = L("MapRotationOffsetDegrees", 0)
			local mapTargetHeading = 0
			if rotation.Mode == "Camera" then
				local camera = workspace.CurrentCamera
				mapTargetHeading = camera and mapPlayerMarkers.MapHeading(camera.CFrame.LookVector, coordinateCosine, coordinateSine, flipX, flipZ) or displayedMapHeading
			elseif rotation.Mode == "Subject" then
				mapTargetHeading = targetHeading - rotationOffset
			end
			displayedMapHeading = mapPlayerMarkers.StepHeading(displayedMapHeading, mapTargetHeading, rotation.Response, dt)
			local mapRotation = -displayedMapHeading
			if mapRotator.Rotation ~= mapRotation then mapRotator.Rotation = mapRotation end
			playerMarker.Rotation = B(defaults, "MapPlayerIconRotates", true) and (displayedPlayerHeading + mapRotation) or mapRotation
			local northMargin = math.max(0, L("MapNorthArrowMargin", 10))
			mapPlayerMarkers.PlaceNorth(northArrow, rotation, mapRotation, mapSize, Vector2.new(1, 1), UDim2.new(1, -northMargin, 1, -northMargin))
			...
			routeRenderer:Step({
				MapVisible = minimap.Visible and leftCluster.Visible and gui.Enabled,
				FullMapStuds = fullMapStuds,
				WorldCenter = Vector2.new(L("MapWorldCenterX", 0), L("MapWorldCenterZ", 0)),
				CoordinateCosine = coordinateCosine,
				CoordinateSine = coordinateSine,
				FlipX = flipX,
				FlipZ = flipZ,
				PixelScale = subpixelFactor,
				MapRotationDegrees = mapRotation,
				LocalWorldPosition = position,
				MapSize = mapSize,
				VisibleStuds = visibleStuds,
			...
			mapIcons:SetVisible(false)
			if routeRenderer then routeRenderer:SetVisible(false) end
		end
	else
		mapPlayerMarkers:SetVisible(false)
		mapIcons:SetVisible(false)
...
local presentationEvent=game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"):FindFirstChild("FreeRoamHudPresentationMode")
...
-- The full map replaces the HUD while open: drop any open dropdown/modal (never the onboarding controls).
player:GetAttributeChangedSignal("FullMapOpen"):Connect(function()
	if player:GetAttribute("FullMapOpen") == true then closeChoiceList(); if activeModal ~= "Controls" then closeModal() end end
end)

pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase1") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2B") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2C") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2D") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2E") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2F") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2G") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2H") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase2I") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase3A") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase3B") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase3C") end)
pcall(function() RunService:UnbindFromRenderStep("PCFreeRoamHudPhase3D") end)
RunService:BindToRenderStep("PCFreeRoamHudPhase4A", 3000, function(dt)
	updateRuntime(dt)
end)
