-- Neo Tokyo Racers - Drive-In Customisation Phase 3
-- Fixes two Phase 2 follow-ups:
-- 1) world prompt is countdown-only and appears only while the vehicle is inside the bay;
-- 2) the normal Start Driving path unlocks the drive-in hold before SpawnVehicle.
--
...
local PHASE3_SPAWN_UNLOCK_MARKER = "NTR_DRIVE_IN_CUSTOMISATION_PHASE3_UNLOCK_BEFORE_SPAWN"

local function info(message)
	print("[NTR Drive-In Customisation Phase 3] " .. tostring(message))
end
...
local function ensureConfig()
	...
	local runtime = ensureFolder(ensureFolder(config, "Runtime"), "DriveInCustomisation")
	local ui = ensureFolder(ensureFolder(config, "UI"), "DriveInCustomisation")

	ensureValue(runtime, "BoolValue", "UseWorldPrompt", true)
	ensureValue(runtime, "BoolValue", "TriggerVfxOnlyWhileDriving", true)
	ensureValue(runtime, "NumberValue", "PromptMaxDistance", 90)
	ensureValue(runtime, "NumberValue", "PromptHeightOffset", 6)
	ensureValue(runtime, "NumberValue", "CountdownSeconds", 3)
	ensureValue(runtime, "NumberValue", "CooldownSeconds", 2)
	ensureValue(runtime, "NumberValue", "PollSeconds", 0.1)

	ensureValue(ui, "StringValue", "PromptPrefix", "Entering customisation in")
	ensureValue(ui, "Color3Value", "PanelColor", Color3.fromRGB(6, 10, 13))
	ensureValue(ui, "Color3Value", "TextColor", Color3.fromRGB(255, 255, 255))
	ensureValue(ui, "Color3Value", "AccentColor", Color3.fromRGB(230, 88, 205))
	ensureValue(ui, "NumberValue", "PanelTransparency", 0.18)
	ensureValue(ui, "NumberValue", "CountdownPromptWidth", 300)
	ensureValue(ui, "NumberValue", "CountdownPromptHeight", 58)
	ensureValue(ui, "NumberValue", "CountdownTextMaxSize", 18)
	ensureValue(ui, "NumberValue", "CountdownTextMinSize", 9)

	for _, object in ipairs(CollectionService:GetTagged(TAG)) do
		if object:IsA("BasePart") then
			object:SetAttribute("PromptPrefix", "Entering customisation in")
		end
	end

	local world = Workspace:FindFirstChild("NeoTokyoRacersWorld")
	local trigger = world
		and world:FindFirstChild("Dealership")
		and world.Dealership:FindFirstChild("Customisation")
		and world.Dealership.Customisation:FindFirstChild("DriveInCustomisationTrigger")
	if trigger and trigger:IsA("BasePart") then
		trigger:SetAttribute("PromptPrefix", "Entering customisation in")
	end

	info("Ensured countdown-only prompt config.")
end
...
local function installClient()
	...
	info("Replaced drive-in client with countdown-only world prompt.")
end
...
local function patchBootstrapSpawnUnlock()
	...
	if string.find(source, PHASE3_SPAWN_UNLOCK_MARKER, 1, true) then
...
info("Phase 3 install complete. Restart Play before testing.")
