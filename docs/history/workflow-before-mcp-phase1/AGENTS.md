...
* Current task is performance Phase 6 acceptance, OPEN; read docs/architecture/performance-phase6-validation.md first for continuation. Phase 5 accepted for continuation. No game code changed during Phase 6. Mirror 2026-09-22 13:14:39 passes exact Phase 5 parity. API-only cycles ran behind the active start-screen state and are not normal gameplay acceptance. CAM-02 and PERF-06-A detached UI references require investigation; actual iPhone 7/multiplayer/soak evidence is unavailable. Do not mark the programme complete or rerun prior installers.
