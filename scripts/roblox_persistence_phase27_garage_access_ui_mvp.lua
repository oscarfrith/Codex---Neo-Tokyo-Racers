...
--
-- Edit mode:
--   Installs an isolated same-server garage access LocalScript.
--   It calls the Phase 23 GarageInteriorInvoke remote and does not patch the
--   main dealership bootstrap.
--
-- Play mode, CLIENT Command Bar:
--   Smoke-checks the UI shell, SetAccessMode, self-visit, state read, and
--   return-to-city.
...
local TAG = "[NTR Persistence Phase 27 Garage Access UI MVP]"
...
if RunService:IsRunning() then
	...
	assert(type(initial) == "table" and initial.Profile ~= nil, "Garage GetInitial failed before Phase 27 smoke.")
	info("Garage GetInitial OK before access UI smoke.")

	local gui = player:WaitForChild("PlayerGui"):WaitForChild("NTR_GarageAccessUI", 8)
	assert(gui and gui:IsA("ScreenGui"), "Garage access UI did not appear in PlayerGui.")
	local toggle = gui:WaitForChild("GarageToggle", 4)
	local panel = gui:WaitForChild("AccessPanel", 4)
	assert(toggle and toggle:IsA("TextButton"), "Garage access toggle missing.")
	assert(panel and panel:IsA("Frame"), "Garage access panel missing.")
	assert(player:GetAttribute("NTR_Phase27GarageAccessUIReady") == true, "Phase 27 UI ready attribute was not set.")
	info("Access UI shell visible. toggle=" .. toggle.Name .. " panel=" .. panel.Name)
	...
	assert(state.VisitingOwnerUserId == player.UserId, "Expected self visit owner state to match local user.")
	info("State OK. inGarage=" .. tostring(state.InGarage) .. " visitingOwner=" .. tostring(state.VisitingOwnerUserId) .. " accessMode=" .. tostring(state.AccessMode))

	local returned = interiorInvoke:InvokeServer("ReturnToCity", { Smoke = true, Phase27 = true })
	...
	info("Expected: access UI shell exists and same-server garage access actions still work. Manually click Enter Mine/Public/Private/Visit/Return to verify controls.")
	return
end
...
local remotes = waitForPath(ReplicatedStorage, { "NeoTokyoRacers", "Shared", "Remotes", "Garage" })
assert(remotes:FindFirstChild("GarageInteriorInvoke"), "Expected Phase 23 GarageInteriorInvoke before Phase 27.")
...
local scriptObject = ensureChild(worldControllers, "LocalScript", "GarageAccessClient_Active")

scriptObject.Source = [=[-- NTR Persistence Phase 27 Garage Access UI MVP
...
scriptObject:SetAttribute("PersistencePhase27GarageAccessUIMVP", true)

info("PASS: installed isolated GarageAccessClient_Active. Restart Play, then run this same script from the CLIENT Command Bar for the smoke test.")
