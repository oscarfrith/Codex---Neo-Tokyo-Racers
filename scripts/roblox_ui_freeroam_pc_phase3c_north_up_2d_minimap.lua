-- Neo Tokyo Racers - PC Free-Roam UI Phase 3C North-Up 2D Minimap
...
-- Phase 2I and installs a hard-clipped north-up 2D minimap assembled from four
-- 1024x1024 image tiles, plus a configurable centred player icon. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 3C North-Up 2D Minimap"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE3C_NORTH_UP_2D_MINIMAP"
...
if controller then
	...
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H or knownPhase2I or knownPhase3A or knownPhase3B or knownPhase3C, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE3C_NORTH_UP_2D_MINIMAP
...
local function buildMainHud()
	...
	playerMarker = new("ImageLabel", { Name = "PlayerMarker", AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = C("Telemetry"), BackgroundTransparency = asset("MapPlayerIcon") == "" and 0 or 1, BorderSizePixel = 0, Image = asset("MapPlayerIcon"), ImageColor3 = C("Text"), ScaleType = Enum.ScaleType.Fit, Position = UDim2.fromScale(0.5, 0.5), Size = UDim2.fromOffset(markerSize, markerSize), ZIndex = 15 }, minimap)
	corner(playerMarker, 4)
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase3C", 3000, function(dt)
	updateRuntime(dt)
end)
]=]
...
info("Installed the canonical isolated Phase 3C north-up 2D minimap.")
info("The stitched ImageLabel canvas never rotates, so ClipsDescendants remains reliable and artwork stays pixel-accurate.")
info("MapCoordinateRotationDegrees defaults to 90; the centred MapPlayerIcon asset and size are configurable.")
info("Phase 3C preserves Phase 2I HUD behavior and does not patch driving, bootstrap, server, VFX, or mobile UI.")
info("Next: restart Play and verify crop, expected location, icon asset/size, heading, and zoom.")
