...
- Mobile thumbstick V2.3 was tested and reported much better, but still needed improvement. Mobile thumbstick V2.4 is generated as the next Studio test: `scripts/roblox_mobile_drive_thumbstick_v2_4_large_edge_drift.lua`. It keeps zero deadzone/linear response, enlarges the inner circle to `1.4x`, makes the outer ring `1.35x` the enlarged inner circle, reduces the invisible hit area to `1.05x`, and moves drift entry near the outer edge with enter/exit thresholds of `0.95`/`0.88`. Verify small-screen fit, boost/thumb overlap, accidental steering, touch jitter, and whether drift is too late or too hard to hold.
...
- The Studio mirror was refreshed on 2026-06-30 at 11:10:17 from Studio and imported 65 scripts. `roblox/studio_snapshot/hierarchy.md` now reflects the thumbstick mobile controller, Lighting Phase AP window material controller, and night lamppost SurfaceLight controller.
