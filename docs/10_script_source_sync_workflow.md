...
4. Run scripts/verify_studio_mirror.py. For the current performance Phase 3 delivery also run scripts/performance_phase3/verify_migration.py. Older cleanup parity verifiers apply only after dependent phases are rolled back. Future intentional changes need their own baseline comparison.
