...
--
-- Adds an isolated local "My Time Trial Bests" board beside the race entry flow.
-- It uses Phase 11N's GetTimeTrialPersonalBest server action and does not patch
-- RaceEntryMenuClient_Active again.
--
-- Scope:
--   Creates/replaces only RacePersonalBestBoardClient_Active.
--   V2 hides the companion board when the race entry menu is closed without starting.
--   No reward config, route-guide config, arrows, VFX, matchmaking, driving,
--   global OrderedDataStore leaderboard, or bootstrap edits.

local PHASE = "NTR Racing Phase 11O V2"
...
local function install()
	...
	local scriptObject = folder:FindFirstChild("RacePersonalBestBoardClient_Active")
	...
	if not scriptObject then
		...
		scriptObject.Name = "RacePersonalBestBoardClient_Active"
		scriptObject.Parent = folder
	end
	if string.find(scriptObject.Source, "NTR_RACING_PHASE11O_TIME_TRIAL_PB_BOARD_V2_MENU_CLOSE_SYNC", 1, true) then
		print("[" .. PHASE .. "] RacePersonalBestBoardClient V2 already installed.")
		return false
	end
	scriptObject.Source = CLIENT_SOURCE
	scriptObject.Disabled = false
	print("[" .. PHASE .. "] Installed RacePersonalBestBoardClient_Active.")
	return true
end

local changed = install()
print("[" .. PHASE .. "] Complete. changed=" .. tostring(changed))
print("[" .. PHASE .. "] Restart Play, open the race entry menu, confirm the local PB board appears, then press EXIT and confirm it closes with the menu.")
