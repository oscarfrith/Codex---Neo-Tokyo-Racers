-- Neo Tokyo Racers - Native garage entrance prompts installer V1
-- NTR_GARAGE_NATIVE_ENTRANCE_PROMPTS_V1
--
-- Run once from the Studio Edit Command Bar, then restart Play.
-- Replaces the isolated canonical garage entrance owner with the same native
-- ProximityPrompt contract used by player vehicles: E on keyboard and a
-- clickable hand/tap prompt on touch devices.

local MODE = "INSTALL" -- INSTALL or AUDIT
local REVISION = "NTR_GARAGE_NATIVE_ENTRANCE_PROMPTS_V1"
local PREFIX = "[NTR Garage Native Entrance Prompts V1]"
...
local introRoot = need(need(clientRoot, "Controllers", "Folder"), "Intro", "Folder")
local controller = need(introRoot, "GarageEntranceController_Active", "LocalScript")
...
local function audit(source)
	...
	check(string.find(source, REVISION, 1, true) ~= nil, "native entrance owner marker installed")
	check(string.find(source, "Enum.ProximityPromptStyle.Default", 1, true) ~= nil, "Roblox native prompt presentation enabled")
	check(string.find(source, [[prompt.ActionText = "Enter"]], 1, true) ~= nil, "shared Enter action text installed")
	check(string.find(source, [[prompt.KeyboardKeyCode = Enum.KeyCode.E]], 1, true) ~= nil, "keyboard E activation installed")
	check(string.find(source, [[prompt.ClickablePrompt = true]], 1, true) ~= nil, "touch click activation installed")
	check(not string.find(source, "NTRCanonicalPrompt", 1, true), "non-interactive custom billboard retired")
	check(not string.find(source, "ProximityPromptStyle.Custom", 1, true), "custom prompt style retired")
	check(string.find(source, [[Mode = "Dealership"]], 1, true) ~= nil and string.find(source, [[Mode = "Customisation"]], 1, true) ~= nil, "dealership and customisation entrances owned")
	check(controller.Disabled == false, "canonical entrance controller enabled")

	print(string.format("%s RESULT %d PASS / %d FAIL", PREFIX, pass, fail))
	return fail == 0
end

compile("GarageEntranceController_Active", newSource)

if MODE == "AUDIT" then
	assert(audit(controller.Source), "Audit failed")
	return
end

assert(MODE == "INSTALL", "MODE must be INSTALL or AUDIT")

local oldSource = controller.Source
local oldDisabled = controller.Disabled
local ok, err = pcall(function()
	controller.Source = newSource
	controller.Disabled = false
	assert(audit(controller.Source), "Post-install audit failed")
end)

if not ok then
	controller.Source = oldSource
	controller.Disabled = oldDisabled
	error("Native entrance prompt install rolled back: " .. tostring(err))
end

print(PREFIX .. " INSTALL COMPLETE - restart Play, then verify E on PC and tap on touch at both entrances.")
