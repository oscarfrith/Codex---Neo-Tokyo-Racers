-- Neo Tokyo Racers - PC Free-Roam UI Phase 2B Canonical Visual Foundation
-- Run in Roblox Studio Command Bar while in Edit mode.
--
-- Canonically replaces only the isolated PC free-roam HUD/controller and
-- refines its semantic visual config. It does not patch the register-limited
-- client bootstrap and does not implement real minimap motion, authoritative
-- dealership teleporting, Developer Product receipts, persisted settings, or
-- the final boost telemetry bridge.

local PHASE = "NTR PC Free-Roam UI Phase 2B Canonical Visual Foundation"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2B_CANONICAL_VISUAL_FOUNDATION"
...
value(layout, "NumberValue", "CarHeaderHeight", 146)
...
if controller then
	...
	local knownPhase2B = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE2B_CANONICAL_VISUAL_FOUNDATION
...
local function buildCarPanel()
	...
	carScroll = new("ScrollingFrame", {
		...
		Position = UDim2.fromOffset(18, L("CarHeaderHeight", 146)), Size = UDim2.new(1, -36, 1, -(L("CarHeaderHeight", 146) + 78)),
...
local function ensureGui()
	...
	gui = new("ScreenGui", { Name = "NTR_DesktopFreeRoamHud", IgnoreGuiInset = false, ResetOnSpawn = false, DisplayOrder = 85, ZIndexBehavior = Enum.ZIndexBehavior.Sibling }, playerGui)
...
local function updateLayout()
	...
	local headerHeight = math.max(L("CarHeaderHeight", 146), 146)
	carScroll.Position = UDim2.fromOffset(innerInset, headerHeight)
	carScroll.Size = UDim2.new(1, -innerInset * 2, 1, -(headerHeight + 78))
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase2B", 3000, function()
	updateRuntime()
end)
]=]
...
info("Installed the canonical isolated Phase 2B controller source.")
info("Preserved existing assets/defaults and added semantic Typography/Effects config groups.")
info("Phase 2B does not patch NeoTokyoRacersClient_Bootstrap_Shadow_Disabled.")
info("Next: restart Play and complete the Phase 2B responsive visual verification checklist.")
