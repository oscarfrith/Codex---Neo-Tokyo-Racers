...
function Controller.Start()
	...
	local UI=require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("RacingUIComponents")); local Mobile=require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("RacingMobileScaledDesktopLayout")); local Shared=require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("GarageComponents")); local remote=game:GetService("ReplicatedStorage"):WaitForChild("Remotes").Garage:WaitForChild("OwnedGarageInvoke"); local push=game:GetService("ReplicatedStorage"):WaitForChild("Remotes").Garage:WaitForChild("OwnedGarageEvent"); local openEvent=game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"):WaitForChild("OpenOwnedGarageBrowser")
	...
	local gui=Instance.new("ScreenGui"); gui.Name="OwnedGarageBrowser"; gui.IgnoreGuiInset=true; gui.ResetOnSpawn=false; gui.DisplayOrder=171; gui.Parent=playerGui
	...
	local browserIcons=game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("GarageReplacement"):WaitForChild("OwnedGarageIcons"):WaitForChild("Browser")
	...
	local function hardenTouch() if not UserInputService.TouchEnabled then return end; local scale=math.max(layoutScale and layoutScale.Scale or 1,.01); local minimum=math.ceil(math.max(32,tonumber(settings:GetAttribute("MinimumTouchTargetPixels")) or 44)/scale); for _,object in ipairs(overlay:GetDescendants()) do if object:IsA("GuiButton") and object:GetAttribute("SkipTouchHardening")~=true then local original=object:GetAttribute("OwnedGarageOriginalSize"); if typeof(original)~="UDim2" then original=object.Size; object:SetAttribute("OwnedGarageOriginalSize",original) end; if original.Y.Scale==0 then object.Size=UDim2.new(original.X.Scale,original.X.Offset,0,math.max(original.Y.Offset,minimum)) end end end end
	...
	local exit=UI.Button(shell,{Name="Exit",Text="EXIT",Position=UDim2.new(0,footerPad,1,footerButtonY),Size=UDim2.new(.5,-(footerPad+footerGap*.5),0,footerButtonHeight),Color=C("PanelSoft"),StrokeColor=C("Outline"),FocusColor=C("Telemetry")}); exit:SetAttribute("SkipTouchHardening",true)
	local enter=UI.Button(shell,{Name="Enter",Text="ENTER GARAGE",Position=UDim2.new(.5,footerGap*.5,1,footerButtonY),Size=UDim2.new(.5,-(footerPad+footerGap*.5),0,footerButtonHeight),Color=C("PanelBlue"),StrokeColor=C("Telemetry"),FocusColor=C("Telemetry")}); enter:SetAttribute("SkipTouchHardening",true)
	-- NTR_GARAGE_VISITS_V1: VISIT tab lists same-server garages whose owner is inside and admits this player.
	local mode="Mine"; local visitRows; local selectedVisit; local tabs={}; local tabWidth=180; local tabHeight=44
	local ACCESS_TEXT={Public="Open to everyone in this server.",FriendsOnly="Open to the owner's friends.",InviteOnly="You are on the owner's invite list."}
	local VISIT_ENDED={OwnerLeft="The owner left the garage. Your visit ended.",AccessRevoked="The owner closed the garage to you.",Disabled="Garage visits are closed right now."}
	local function toast(text) local ui=player:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"); local event=ui:FindFirstChild("ShowTopNotification"); if event and event:IsA("BindableEvent") then event:Fire(text,2.2) end end
	local function inVehicle() local character=player.Character; local humanoid=character and character:FindFirstChildOfClass("Humanoid"); return humanoid~=nil and humanoid.SeatPart~=nil end
	local function setEnterEnabled(on) enter.Active=on==true; enter.TextColor3=on and C("Text") or C("Muted") end


	local function presentation(active) local event=game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"):FindFirstChild("FreeRoamHudPresentationMode"); if event and event:IsA("BindableEvent") then event:Fire({Owner="OwnedGarageBrowser",Active=active==true,KeepTelemetry=false}) end end
	...
	local function loadingAction(action,payload) local ok,result=pcall(function() return loadingInvoke:Invoke(action,payload or {}) end); if ok then return result end; warn("[Owned Garage] Loading transition "..tostring(action).." failed: "..tostring(result)); return nil end
	...
	local function renderDetail()
		...
		detailTitle.Text=string.upper(selected.DisplayName or selected.PropertyId); district.Text=string.upper(selected.District or ""); description.Text=tostring(selected.Description or ""); heroImage.Image=UI.Asset(selected.Image or ""); heroImage.Visible=heroImage.Image~=""; placeholder.Visible=not heroImage.Visible; capacityText.Text=tostring(selected.Filled or 0).." / "..tostring(selected.Capacity or 0).." DISPLAY SPACES"; enter.Visible=true; setEnterEnabled(true); Shared.SetActionButton(enter,state and state.Visiting and "LEAVE GARAGE" or (state and state.InGarage and "RETURN TO CITY" or "ENTER GARAGE"),nil,state and state.InGarage and "↩" or "E")
	end
	local setMode; local refreshVisits
	local function renderTabs()
		for _,tab in ipairs(tabs) do tab:Destroy() end; table.clear(tabs); if not (state and (state.VisitsEnabled==true or state.Visiting~=nil)) then if mode=="Visit" then mode="Mine"; titleLabel.Text="MY GARAGES" end; return end
		for index,tab in ipairs({{Mode="Mine",Text="MY GARAGES"},{Mode="Visit",Text="VISIT"}}) do local active=mode==tab.Mode; local button=UI.Button(shell,{Name=tab.Mode.."Tab",Text=tab.Text,Position=UDim2.new(1,-(footerPad+tabWidth*(3-index)+footerGap*(2-index)),0,10),Size=UDim2.fromOffset(tabWidth,tabHeight),Color=active and C("PanelBlue") or C("PanelSoft"),StrokeColor=active and C("Telemetry") or C("Outline"),FocusColor=C("Telemetry")}); button:SetAttribute("SkipTouchHardening",true); button.Activated:Connect(function() if not busy and mode~=tab.Mode then setMode(tab.Mode) end end); table.insert(tabs,button) end
	end
	local function renderVisitDetail()
		local row=selectedVisit
		if not row then detailTitle.Text=visitRows and "NO OPEN GARAGES" or "LOADING..."; district.Text=""; description.Text=visitRows and "Garages appear here while their owner is inside and has opened them to you." or ""; capacityText.Text=""; heroImage.Visible=false; placeholder.Visible=true; enter.Visible=state~=nil and state.Visiting~=nil; if enter.Visible then setEnterEnabled(true); Shared.SetActionButton(enter,"LEAVE GARAGE") end; return end
		detailTitle.Text=string.upper(row.GarageName or ""); district.Text="HOSTED BY "..string.upper(row.OwnerName or ""); description.Text=(ACCESS_TEXT[row.AccessMode] or "")..((row.Message or "")~="" and (" "..row.Message) or ""); heroImage.Image=UI.Asset(row.Image or ""); heroImage.Visible=heroImage.Image~=""; placeholder.Visible=not heroImage.Visible; capacityText.Text=tostring(row.VisitorCount or 0).." / "..tostring(row.MaxVisitors or 0).." VISITORS"; enter.Visible=true
		if row.Visiting then setEnterEnabled(true); Shared.SetActionButton(enter,"LEAVE GARAGE") elseif inVehicle() then setEnterEnabled(false); Shared.SetActionButton(enter,"EXIT YOUR CAR TO VISIT") elseif not row.CanVisit then setEnterEnabled(false); Shared.SetActionButton(enter,row.Message or "UNAVAILABLE") else setEnterEnabled(true); Shared.SetActionButton(enter,"VISIT") end
	end
	local function renderVisit()
		for index,row in ipairs(visitRows or {}) do local card=Shared.Card(list,{Name="Visit_"..tostring(row.OwnerUserId),DisplayName=string.upper(tostring(row.OwnerName or "")),Image=UI.Asset(row.Image or ""),Rating=tostring(row.VisitorCount or 0).."/"..tostring(row.MaxVisitors or 0),Selected=selectedVisit~=nil and selectedVisit.OwnerUserId==row.OwnerUserId,Muted=row.CanVisit~=true and row.Visiting~=true,Size=UDim2.new(1,0,0,158),ImageHeight=148}); card.LayoutOrder=index; card.Activated:Connect(function() selectedVisit=row; render() end); cards["Visit_"..tostring(row.OwnerUserId)]=card end; renderVisitDetail()
	end
	render=function()
		renderTabs(); if mode=="Visit" then clearList(); renderVisit(); return end
	...
	refreshVisits=function() local token=generation; task.spawn(function() local result=request("GetVisitableGarages",{}); if token~=generation or mode~="Visit" then return end; if not result.Success then setStatus(result.Message,false); return end; visitRows=result.Garages or {}; local keep=selectedVisit and selectedVisit.OwnerUserId; selectedVisit=nil; for _,row in ipairs(visitRows) do if row.OwnerUserId==keep then selectedVisit=row end end; selectedVisit=selectedVisit or visitRows[1]; render(); task.defer(hardenTouch) end) end
	setMode=function(newMode) mode=newMode; titleLabel.Text=mode=="Visit" and "VISIT GARAGES" or "MY GARAGES"; setStatus(""); if mode=="Visit" then visitRows=nil; selectedVisit=nil; render(); refreshVisits() else render() end end
	local function visitAction()
		if busy then return end; local leaving=state~=nil and state.Visiting~=nil and (mode~="Visit" or not selectedVisit or selectedVisit.Visiting==true); if not leaving and not (mode=="Visit" and selectedVisit and selectedVisit.CanVisit and not inVehicle()) then return end; busy=true
		local loadingGeneration=loadingAction("Begin",{Destination=leaving and "OwnedGarageExterior" or "OwnedGarageInterior",Status=leaving and "RETURNING TO CITY" or "VISITING GARAGE"}); local result; if leaving then result=request("LeaveVisit",{}) else result=request("VisitGarage",{OwnerUserId=selectedVisit.OwnerUserId}) end; busy=false
		if result.Success then close(); loadingAction("Complete",{Generation=loadingGeneration,Status="READY"}) else loadingAction("Fail",{Generation=loadingGeneration,Status="RETURNING",Reason=result.Message}); setStatus(result.Message,false); if mode=="Visit" then refreshVisits() end end
	end
	...
	ProximityPromptService.PromptTriggered:Connect(function(prompt,triggeringPlayer)
		...
		if player:GetAttribute("OwnedGarageInside")==true then
			if prompt.Name=="FootExitPrompt" then beginPhysicalLoading("OwnedGarageExterior","RETURNING TO CITY") elseif prompt.Name=="DriveOutPrompt" and player:GetAttribute("OwnedGarageVisitor")~=true then beginPhysicalLoading("OwnedGarageDriveOut","PREPARING VEHICLE") end
	...
	player:GetAttributeChangedSignal("OwnedGarageInside"):Connect(function() if player:GetAttribute("OwnedGarageInside")~=true then finishPhysicalLoading(true,"Ready") end end)
	local function destinationReady(message)
		if message.DestinationType=="Interior" then local world=game:GetService("Workspace"):FindFirstChild("World"); local interiors=world and game:GetService("Workspace"):WaitForChild("World"):FindFirstChild("Interiors"); local pool=interiors and game:GetService("Workspace"):WaitForChild("World"):WaitForChild("Interiors"):FindFirstChild("OwnedGarageInstances"); local model=pool and pool:FindFirstChild(tostring(message.InteriorName or "")); return model and model:FindFirstChild(tostring(message.MarkerName or "CharacterSpawn"),true)~=nil end
		if message.DestinationType=="Exterior" then local world=game:GetService("Workspace"):FindFirstChild("World"); local root=world and game:GetService("Workspace"):WaitForChild("World"):FindFirstChild("OwnedGarageExteriors"); local exterior=root and root:FindFirstChild(tostring(message.ExteriorId or "")); return exterior and exterior:FindFirstChild(tostring(message.MarkerName or ""),true)~=nil end
		return false
	end
	push.OnClientEvent:Connect(function(message)
		...
		elseif message.Type=="FootExitResult" then finishPhysicalLoading(message.Success==true,message.Message)
		elseif message.Type=="VisitEnded" then toast(VISIT_ENDED[tostring(message.Reason or "")] or "Your garage visit ended."); finishPhysicalLoading(true,"Ready"); if state then state.Visiting=nil end; if overlay.Visible then if mode=="Visit" then refreshVisits() else render() end end end
	end)
	started=true; print("[Owned Garage] Browser controller active."); return true,"Started"
end
return Controller
