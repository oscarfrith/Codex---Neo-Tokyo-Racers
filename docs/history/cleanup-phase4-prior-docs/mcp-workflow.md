...
7. Start `scripts/receive_studio_snapshot.py` locally, then execute the exact `scripts/studio_export_snapshot.lua` in Edit through MCP. Receiver is loopback-only and handles one export. If Python/py is missing, discover the bundled Python using the app's workspace dependencies tool. Do not hardcode one machine's runtime in project scripts.
