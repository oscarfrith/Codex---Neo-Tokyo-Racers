...
- 2026-06-30: Added `scripts/roblox_vfx_mobile_late_socket_rescan_repair.lua` for intermittent missing engine/boost/stabiliser VFX on mobile. The likely root cause is that `VehicleVFXController.Attach` only scans VFX sockets and captures the vehicle root once; the repair tracks handled sockets, refreshes a late root, and periodically attaches templates to late-replicated sockets without duplicating existing hosts.
