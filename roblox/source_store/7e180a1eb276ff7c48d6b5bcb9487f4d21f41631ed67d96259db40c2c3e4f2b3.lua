...
function Module.ResolveRotation(config, localPlayer)
	local mode = tostring(config:GetAttribute("MapRotationMode") or "Camera")
	if not ROTATION_MODES[mode] then mode = "Camera" end
	...
	local northMode = tostring(config:GetAttribute("MapNorthArrowMode") or "Orbit")
	if northMode ~= "Orbit" and northMode ~= "Corner" then northMode = "Orbit" end
	return {
		Mode = mode,
...
-- Places the north indicator: on the rim when the map rotates, otherwise at
