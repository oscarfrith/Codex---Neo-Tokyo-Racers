...
- The Phase 17 action-rail workaround is not the desired UX. It kept the carousel clipped, but the user rejected it because BUY/LOCKED/EQUIP appeared centred above the module frame/screen instead of above the clicked module card. Stop patching from stale context, refresh the Studio mirror, and use `docs/persistence-garage-phase17-ui-handoff-2026-07-02.md` before another popup repair.
...
- The mirror is stale after the Studio-confirmed Persistence Phase 1-17 installs and the later popup repair attempts until the full snapshot workflow captures the actual live `NeoTokyoRacersClient_Bootstrap_Shadow_Disabled`, `GarageActionController_Shadow_Disabled`, ProfileService/data modules, garage property controller/catalog, instance inventory bridge, module source-lock/sorting/tab changes, colour picker recovery, front-engine catalogue repair, and current rejected action-rail state.
