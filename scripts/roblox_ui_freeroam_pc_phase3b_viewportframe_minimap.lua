-- Neo Tokyo Racers - PC Free-Roam UI Phase 3B ViewportFrame Minimap
...
-- Phase 3A and replaces the rotated 2D map canvas with a hard-cropped
-- ViewportFrame camera over four textured tiles. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 3B ViewportFrame Minimap"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE3B_VIEWPORTFRAME_MINIMAP"
...
value(layout, "NumberValue", "BoostBarOffsetY", 67).Value = 67
value(layout, "NumberValue", "BoostBarSmoothing", 14)
value(layout, "NumberValue", "BoostIconSize", 32)
value(layout, "NumberValue", "BoostIconGap", 8)
value(layout, "NumberValue", "MapPixels", 2048)
value(layout, "NumberValue", "MapCalibrationPixels", 207)
value(layout, "NumberValue", "MapCalibrationStuds", 2850)
value(layout, "NumberValue", "MapWorldCenterX", 0)
value(layout, "NumberValue", "MapWorldCenterZ", 0)
value(layout, "NumberValue", "MapVisibleStuds", 2850)
value(layout, "NumberValue", "MapRotationOffsetDegrees", 0)
value(layout, "NumberValue", "MapCoordinateRotationDegrees", 90).Value = 90
value(layout, "NumberValue", "MapSmoothing", 10)
value(layout, "NumberValue", "MapViewportFieldOfView", 20)
...
value(defaults, "BoolValue", "MapFlipX", false)
value(defaults, "BoolValue", "MapFlipZ", false)
...
if controller then
	...
	local knownPhase2G = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE2G_LIVE_BOOST_TELEMETRY", 1, true)
	local knownPhase2H = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE2H_CONFIGURABLE_BOOST_ICON", 1, true)
	local knownPhase2I = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE2I_BORDERLESS_BOOST_ICON", 1, true)
	local knownPhase3A = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE3A_FOUR_TILE_LIVE_MINIMAP", 1, true)
	local knownPhase3B = string.find(controller.Source, "NTR_PC_FREEROAM_UI_PHASE3B_VIEWPORTFRAME_MINIMAP", 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H or knownPhase2I or knownPhase3A or knownPhase3B, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE3B_VIEWPORTFRAME_MINIMAP
...
local moneyLabel
local minimap
local mapViewportFrame
local mapCamera
local playerArrow
local mapMissingLabel
local displayedMapPosition
local displayedMapRotation
local bottomActions
...
local function buildMainHud()
	...
	corner(minimap, 9)
	mapViewportFrame = new("ViewportFrame", { Name = "MapViewport", Ambient = Color3.new(1, 1, 1), LightColor = Color3.new(1, 1, 1), LightDirection = Vector3.new(0, -1, 0), BackgroundColor3 = C("PanelDeep"), BackgroundTransparency = 0, BorderSizePixel = 0, Size = UDim2.fromScale(1, 1), ZIndex = 9 }, minimap)
	corner(mapViewportFrame, 9)
	local mapWorld = new("WorldModel", { Name = "MapWorld" }, mapViewportFrame)
	mapCamera = new("Camera", { Name = "MapCamera", FieldOfView = L("MapViewportFieldOfView", 20) }, mapViewportFrame)
	mapViewportFrame.CurrentCamera = mapCamera
	local tileNames = { "MapTileTopLeft", "MapTileTopRight", "MapTileBottomLeft", "MapTileBottomRight" }
	local tilePositions = { Vector3.new(-512, 0, -512), Vector3.new(512, 0, -512), Vector3.new(-512, 0, 512), Vector3.new(512, 0, 512) }
	...
	for index, tileName in ipairs(tileNames) do
		...
		local tile = new("Part", { Name = tileName, Anchored = true, CanCollide = false, CanQuery = false, CanTouch = false, CastShadow = false, Color = C("PanelDeep"), Material = Enum.Material.SmoothPlastic, Position = tilePositions[index], Size = Vector3.new(1024, 0.05, 1024) }, mapWorld)
		new("Decal", { Name = "MapTexture", Face = Enum.NormalId.Top, Texture = image, Color3 = Color3.new(1, 1, 1) }, tile)
	end
	...
	playerArrow = label(minimap, "PlayerArrow", "^", UDim2.fromOffset(34, 34), UDim2.new(0.5, -17, 0.5, -17), 25, C("Telemetry"), Enum.TextXAlignment.Center)
	playerArrow.ZIndex = 15
...
local function updateRuntime(dt)
	...
	if mapSubject and mapSubject:IsA("BasePart") then
		...
		local studsPerPixel = calibrationStuds / calibrationPixels
		...
		local heading = math.deg(math.atan2(mappedLookX, -mappedLookZ))
		local mode = string.upper(tostring(readValue(defaults, "Minimap", "ROTATE")))
		local targetRotation = heading + L("MapRotationOffsetDegrees", 0)
		local targetPosition = Vector2.new(mappedX / studsPerPixel, mappedZ / studsPerPixel)
		...
		if displayedMapRotation == nil then displayedMapRotation = targetRotation end
		local rotationDelta = (targetRotation - displayedMapRotation + 180) % 360 - 180
		displayedMapRotation += rotationDelta * alpha
		local visiblePixels = visibleStuds / studsPerPixel
		local fieldOfView = math.clamp(L("MapViewportFieldOfView", 20), 5, 70)
		mapCamera.FieldOfView = fieldOfView
		local cameraHeight = (visiblePixels * 0.5) / math.tan(math.rad(fieldOfView * 0.5))
		local cameraHeading = mode == "ROTATE" and displayedMapRotation or 0
		local headingRadians = math.rad(cameraHeading)
		local cameraUp = Vector3.new(math.sin(headingRadians), 0, -math.cos(headingRadians))
		local cameraPosition = Vector3.new(displayedMapPosition.X, cameraHeight, displayedMapPosition.Y)
		mapCamera.CFrame = CFrame.lookAt(cameraPosition, Vector3.new(displayedMapPosition.X, 0, displayedMapPosition.Y), cameraUp)
		playerArrow.Rotation = mode == "ROTATE" and 0 or displayedMapRotation
	end
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase3B", 3000, function(dt)
	updateRuntime(dt)
end)
]=]
...
info("Installed the canonical isolated Phase 3B ViewportFrame minimap.")
info("ViewportFrame hard-crops four textured 1024px tiles; rotated 2D UI no longer escapes the box.")
info("MapCoordinateRotationDegrees defaults to 90 to match the supplied green/red/yellow alignment evidence.")
info("Phase 3B preserves Phase 2I HUD behavior and does not patch driving, bootstrap, server, VFX, or mobile UI.")
info("Next: restart Play and verify crop, tile texture orientation, position, rotation, and zoom.")
