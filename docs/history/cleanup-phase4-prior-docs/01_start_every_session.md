...
7. If the task touches a specific area, also read the relevant topic docs:
...
9. If live Studio script source or hierarchy matters, check `roblox/exported_scripts/` and `roblox/studio_snapshot/`, then say whether either appears stale.
10. Check Git status/diff if local repo access is available.
...
- After any Roblox Studio change that affects scripts, hierarchy, assets/folders, services, config attributes, or live object placement, refresh the repo mirror before final handoff whenever practical.
- Use `py scripts/receive_studio_snapshot.py` locally, then run `scripts/studio_export_snapshot.lua` in the Roblox Studio Command Bar.
...
- Keep old failed experiments out of the current baseline unless intentionally kept for history.
