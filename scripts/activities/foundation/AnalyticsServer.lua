...
Service.Events = {
	...
	Currency = "Cash",
	Custom = { RankUp = true, ActivityCompleted = true, DuelResult = true },
}
...
-- Street Life custom events (registry above is the allowlist). value is a number; fields are custom fields.
function Service.Custom(player, name, value, fields)
	if not (Service.Events.Custom[tostring(name)] and livePlayer(player)) then return end
	send("custom", function()
		game:GetService("AnalyticsService"):LogCustomEvent(player, tostring(name), tonumber(value) or 1, type(fields) == "table" and fields or nil)
	end)
end
