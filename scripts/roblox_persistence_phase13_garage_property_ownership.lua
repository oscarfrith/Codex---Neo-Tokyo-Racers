-- Neo Tokyo Racers - Persistence Phase 13
-- Merged property-ownership step:
-- - Adds real BuyGarageProperty server action.
-- - Stores owned garage property IDs on the current V56 session profile.
-- - Calculates garage capacity from owned properties, while preserving old Phase 7 upgrades.
-- - Mirrors owned garage properties into ProfileService snapshots.
-- - Updates the extracted GaragePropertyMenuController to call BuyGarageProperty.
--
-- Run from Roblox Studio Command Bar in Edit mode after Phase 12 has been tested.
-- This uses guarded source replacement against the active garage controller and client bootstrap.

local ReplicatedStorage = game:GetService("ReplicatedStorage")
local ServerScriptService = game:GetService("ServerScriptService")
local StarterPlayer = game:GetService("StarterPlayer")

local PHASE = "Persistence Phase 13"
...
local function replaceOnce(source, oldText, newText, label)
	...
	if not first then
		error("Could not find source anchor for " .. label .. ". Refresh the Studio mirror before making another Phase 13 patch.")
	end
...
local function setSourceMarker(instance, attributeName)
	instance:SetAttribute(attributeName, true)
end

local ntr = assertChild(ReplicatedStorage, "NeoTokyoRacers")
local dataModules = waitPath(ntr, "Shared", "Modules", "Data")
local catalog = assertChild(dataModules, "GaragePropertyCatalog")
assert(catalog:IsA("ModuleScript"), "GaragePropertyCatalog must be a ModuleScript.")
...
setSourceMarker(catalog, "PersistencePhase13GaragePropertyOwnership")

local schema = assertChild(dataModules, "PlayerProfileSchema")
assert(schema:IsA("ModuleScript"), "PlayerProfileSchema must be a ModuleScript.")
local schemaSource = schema.Source
if not string.find(schemaSource, "NTR_PERSISTENCE_PHASE13_GARAGE_PROPERTIES", 1, true) then
	schemaSource = replaceOnce(schemaSource, [[		AccessMode = "Private",
		InvitedUserIds = {},
]], [[		AccessMode = "Private",
		-- NTR_PERSISTENCE_PHASE13_GARAGE_PROPERTIES
		OwnedGarageProperties = {},
		InvitedUserIds = {},
]], "schema default owned garage properties")

	schemaSource = replaceOnce(schemaSource, [[	profile.Garage.InvitedUserIds = typeof(profile.Garage.InvitedUserIds) == "table" and profile.Garage.InvitedUserIds or {}
	...
	profile.Garage.Customisation = normalizeGarageCustomisation(profile.Garage.Customisation)
]], "schema normalize owned garage properties")

	schemaSource = replaceOnce(schemaSource, [[		VehicleCount = countDictionary(profile.Vehicles),
		...
		CockpitInstanceCount = countDictionary(profile.OwnedCockpitInstances),
]], "schema summary garage property count")

	schema.Source = schemaSource
	info("Patched PlayerProfileSchema with Garage.OwnedGarageProperties.")
else
	info("PlayerProfileSchema already has Phase 13 garage property support.")
end

local mapper = assertChild(dataModules, "LegacyGarageProfileMapper")
assert(mapper:IsA("ModuleScript"), "LegacyGarageProfileMapper must be a ModuleScript.")
local mapperSource = mapper.Source
if not string.find(mapperSource, "NTR_PERSISTENCE_PHASE13_GARAGE_PROPERTIES", 1, true) then
	mapperSource = replaceOnce(mapperSource, [[	local preserveLegacyCapacity = options.PreserveLegacyCapacity ~= false
...
]], "mapper owned garage properties")
	mapper.Source = mapperSource
	info("Patched LegacyGarageProfileMapper to mirror owned garage properties.")
else
	info("LegacyGarageProfileMapper already mirrors owned garage properties.")
end
...
assert(string.find(garageSource, "NTR_PERSISTENCE_PHASE7_GARAGE_CAPACITY_UPGRADE", 1, true), "Run Persistence Phase 7 before Phase 13.")

if not string.find(garageSource, "NTR_PERSISTENCE_PHASE13_GARAGE_PROPERTIES", 1, true) then
	garageSource = replaceOnce(garageSource, [[			GarageCapacity = 2,
			ModuleUpgradeLevels = {},
]], [[			GarageCapacity = 2,
			-- NTR_PERSISTENCE_PHASE13_GARAGE_PROPERTIES
			OwnedGarageProperties = {},
	...
	garageSource = replaceOnce(garageSource, [[		profile.GarageCapacity = math.max(1, math.floor(tonumber(profile.GarageCapacity) or 2))
	...
	garageSource = replaceOnce(garageSource, [[		local V80_mutatingActions = {
		BuyCockpit = true,
]], [[		local V80_mutatingActions = {
		BuyCockpit = true,
		BuyGarageProperty = true,
]], "garage mirror mutating action")

	garageSource = replaceOnce(garageSource, [[	local function V82_profileGarageCapacity(profile)
	...
	garageSource = replaceOnce(garageSource, [[	local function V82_upgradeGarageCapacity(profile)
		if not profile then
	...
	garageSource = replaceOnce(garageSource, [[			Garage = {
	...
	garageSource = replaceOnce(garageSource, [[			elseif action == "UpgradeGarageCapacity" then
	...
	info("Patched GarageActionController_Shadow_Disabled with BuyGarageProperty.")
else
	info("GarageActionController_Shadow_Disabled already has Phase 13 garage property ownership.")
end
...
assert(string.find(bootstrapSource, "NTR_PERSISTENCE_PHASE12_GARAGE_MENU_CONTROLLER", 1, true), "Run and test Persistence Phase 12 before Phase 13.")
if not string.find(bootstrapSource, "NTR_PERSISTENCE_PHASE13_GARAGE_PROPERTIES", 1, true) then
	...
	info("Patched client bootstrap context for GaragePropertyMenuController.")
else
	info("Client bootstrap already passes State into GaragePropertyMenuController.")
end

local uiControllers = waitPath(clientRoot, "Controllers", "UI")
local menuController = assertChild(uiControllers, "GaragePropertyMenuController")
...
setSourceMarker(menuController, "PersistencePhase13GaragePropertyOwnership")

local okCatalog, catalogModule = pcall(require, catalog)
assert(okCatalog and type(catalogModule) == "table" and type(catalogModule.List) == "function", "GaragePropertyCatalog did not require after Phase 13 patch.")

local okMenu, menuModule = pcall(require, menuController)
assert(okMenu and type(menuModule) == "table" and type(menuModule.Render) == "function", "GaragePropertyMenuController did not require after Phase 13 patch.")

info("PASS: Phase 13 merged garage property ownership patch installed.")
