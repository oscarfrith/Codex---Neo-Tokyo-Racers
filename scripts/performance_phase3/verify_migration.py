...
data,source,_=project(current)
generated=next(r for r in manifest if r['path_parts'][-1]=='VehicleCatalogData')
fresh=source==(ROOT/generated['file']).read_text(encoding='utf8')
result=dict(mirror=current['generated_in_studio'],sources=len(actual),source_sha256_parity=source_ok,all_hierarchy_property_parity=left==right,catalogue_fresh=fresh,nodes=sum(right.values()),replicatedDescendants=sum(n['path_parts'][0]=='ReplicatedStorage' and len(n['path_parts'])>1 for n in flat(current)),missing_or_changed=sum((left-right).values()),added_or_changed=sum((right-left).values()))
...
print(json.dumps(result,indent=2));assert source_ok and left==right and fresh
