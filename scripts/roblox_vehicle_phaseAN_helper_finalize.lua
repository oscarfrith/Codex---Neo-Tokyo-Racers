-- Neo Tokyo Racers - Phase AN helper install/finalization
-- Run in Edit mode. Does not patch the garage controller.

local ReplicatedStorage = game:GetService("ReplicatedStorage")

local performance = ReplicatedStorage
...
if helper and not helper:IsA("ModuleScript") then
	error(helper:GetFullName() .. " must be a ModuleScript")
end
if not helper then
	helper = Instance.new("ModuleScript")
	helper.Name = "VehicleModuleUpgradeRuntime"
	helper.Parent = performance
end
...
assert(string.find(helper.Source, "function Runtime.Purchase", 1, true), "Helper Purchase function was not retained")
assert(string.find(helper.Source, "function Runtime.ApplyToClone", 1, true), "Helper ApplyToClone function was not retained")
assert(string.find(helper.Source, "function Runtime.CatalogForModuleType", 1, true), "Helper CatalogForModuleType function was not retained")

print("[NTR Vehicle Phase AN Repair] Helper installed, source verified, and Phase set to AN.")
