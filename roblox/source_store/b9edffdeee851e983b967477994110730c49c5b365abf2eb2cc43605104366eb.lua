...
local LOG_PREFIX = "[Dealership Intro Progress]"
local COMPLETE_ATTRIBUTE = "DealershipIntroObjectiveComplete"
local LOADED_ATTRIBUTE = "DealershipIntroObjectiveLoaded"
...
local function getIntro()
	...
	return dealership and game:GetService("Workspace"):WaitForChild("World"):WaitForChild("Dealership"):FindFirstChild("Intro")
end
...
local Net=require(game:GetService("ServerStorage"):WaitForChild("Modules"):WaitForChild("Core"):WaitForChild("Net")); local handleIntroGetComplete; getCompleteRemote.OnServerInvoke = Net.invoke({name="GetDealershipIntroObjective",withAction=false,reply="boolean",capacity=20,refill=5}, function(...) return handleIntroGetComplete(...) end); handleIntroGetComplete = function(player)
	return waitForLoaded(player)
end

local Net=require(game:GetService("ServerStorage"):WaitForChild("Modules"):WaitForChild("Core"):WaitForChild("Net")); local handleIntroComplete; completeRemote.OnServerEvent:Connect(Net.event({name="CompleteDealershipIntroObjective",capacity=10,refill=2}, function(...) return handleIntroComplete(...) end)); handleIntroComplete = (function(player)
