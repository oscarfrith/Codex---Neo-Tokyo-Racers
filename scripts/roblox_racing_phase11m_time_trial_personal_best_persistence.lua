-- Neo Tokyo Racers - Racing Phase 11M Time Trial Personal Best Persistence
-- Run in Roblox Studio Command Bar, Edit mode, then restart Play.
--
-- Adds a dedicated RacingPersonalBestService and patches TimeTrialService to use
-- it when deciding whether a time-trial result is a new PB.
--
-- DataStore writes are configurable and default OFF for prototype safety:
--   ReplicatedStorage.NeoTokyoRacers.Config.Racing.PersonalBests.DataStoreEnabled
--
-- Scope:
--   Server-side racing PB service + guarded TimeTrialService patch only.
--   No UI layout, rewards config, route-guide config, arrows, matchmaking, VFX,
--   driving, or bootstrap edits.

local PHASE = "NTR Racing Phase 11M"
...
local PERSONAL_BEST_SERVICE_SOURCE = [==[
...
local function ensurePersonalBestConfig()
	local ntr = game:GetService("ReplicatedStorage"):WaitForChild("NeoTokyoRacers")
	local config = ensureFolder(ntr:WaitForChild("Config"), "Racing")
	local pb = ensureFolder(config, "PersonalBests")
	ensureValue(pb, "BoolValue", "DataStoreEnabled", false)
	ensureValue(pb, "StringValue", "DataStoreName", "NTR_TimeTrialPersonalBests_v1")
	ensureValue(pb, "NumberValue", "SaveDebounceSeconds", 6)
	print("[" .. PHASE .. "] Ensured Config.Racing.PersonalBests.")
end

local function installPersonalBestService()
	local sss = game:GetService("ServerScriptService")
	local serverRoot = ensureFolder(sss, "NeoTokyoRacers")
	local services = ensureFolder(serverRoot, "Services")
	local racing = ensureFolder(services, "Racing")
	local scriptObject = racing:FindFirstChild("RacePersonalBestService_Active")
	...
	if not scriptObject then
		...
		scriptObject.Name = "RacePersonalBestService_Active"
		scriptObject.Parent = racing
	end
	scriptObject.Disabled = false
	scriptObject.Source = PERSONAL_BEST_SERVICE_SOURCE
	print("[" .. PHASE .. "] Installed RacePersonalBestService_Active.")
end
...
local function patchTimeTrialService()
	...
	if string.find(source, "NTR_RACING_PHASE11M_PERSISTENT_PB_HELPERS", 1, true) then
		print("[" .. PHASE .. "] TimeTrialService already patched for persistent PBs.")
		return false
	end
...
local function recordPersistentPersonalBest(player, run, elapsed, medal)
	...
	return { Ok = false, Message = "Persistent PB service failed: " .. tostring(result) }
		...
		"persistent PB helper insertion"
	)
	...
	local reward = grantTimeTrialReward(player, run, elapsed, medal, isPersonalBest)
		...
		"sendTimeTrialResult PB decision block"
	)

	scriptObject.Source = source
	print("[" .. PHASE .. "] Patched TimeTrialService to record persistent PBs.")
	return true
end

ensurePersonalBestConfig()
installPersonalBestService()
local patchedTimeTrial = patchTimeTrialService()

print("[" .. PHASE .. "] Complete. patchedTimeTrial=" .. tostring(patchedTimeTrial))
print("[" .. PHASE .. "] Restart Play. Finish a time trial twice: a slower second run should keep the first PB; a faster run should show NEW PERSONAL BEST.")
