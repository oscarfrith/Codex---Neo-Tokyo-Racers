...
function Client.start()
    ...
    local function readState()
        ...
        assert(incoming.Version==1 and Cycle.finite(incoming.Epoch),"Invalid lighting state")
        local built=Cycle.build(presets,schedule,definition,incoming.BaseDurationSeconds,incoming.Latitude)
        ...
        assert(type(incoming.Auto)=="boolean" and built.byName[incoming.ManualStage],"Invalid manual state")
    ...
    local function render()
        ...
        local seconds=state.Auto and (workspace:GetServerTimeNow()-state.Epoch) or cycle.byName[state.ManualStage].start
        local preview=RunService:IsStudio() and tools:GetAttribute("LightingPreviewEnabled")==true and runtime:GetAttribute("LightingPreviewCycleSeconds")
        local target,info=Cycle.sample(cycle,Cycle.finite(preview) and preview or seconds)
        local owner="City"
