...
function Payout.Pay(player, spec)
	...
	local existing = paid[player][spec.CommandId]
	if existing then
		-- A concurrent call with the same CommandId waits for the first one instead of reporting success early.
		local deadline = os.clock() + 15
		while existing.InFlight and os.clock() < deadline and player.Parent == Players do task.wait(0.1) end
		if existing.InFlight then return { Ok = false, Cash = 0, Xp = 0, Capped = false, Message = "Payout still in progress." } end
		return existing
	end
	local result = { Ok = true, Cash = 0, Xp = 0, Capped = false, InFlight = true }
