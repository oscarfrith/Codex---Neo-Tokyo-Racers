-- Neo Tokyo Racers - PC Free-Roam UI Phase 2E Vehicle Grid Internal Padding
...
-- Phase 2D and fixes the remaining first-row stroke clipping at its root by
-- padding inside the clipped scrolling frame. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 2E Vehicle Grid Internal Padding"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2E_VEHICLE_GRID_INTERNAL_PADDING"
...
if controller then
	...
	local knownPhase2E = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE2E_VEHICLE_GRID_INTERNAL_PADDING
...
local function buildCarPanel()
	...
	carGridPadding = new("UIPadding", {
		Name = "CardStrokePadding",
		PaddingTop = UDim.new(0, L("CardTopSafePadding", 8)),
	}, carScroll)
...
local function updateLayout()
	...
	carGridPadding.PaddingTop = UDim.new(0, cardTopSafePadding)
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase2E", 3000, function()
	updateRuntime()
end)
]=]
...
info("Installed the canonical isolated Phase 2E vehicle-grid internal-padding repair.")
info("CardTopSafePadding now pads inside VehicleGrid instead of moving its clipping boundary with the cards.")
info("Phase 2E preserves the confirmed Phase 2D component polish and does not patch the bootstrap.")
info("Next: restart Play and confirm the complete first-row top stroke/glow is visible.")
