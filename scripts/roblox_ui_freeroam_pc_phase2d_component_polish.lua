-- Neo Tokyo Racers - PC Free-Roam UI Phase 2D Component Polish
-- Run in Roblox Studio Command Bar while in Edit mode.
--
-- Canonically replaces only the isolated PC free-roam HUD/controller to polish
-- card clearance, dropdown toggling/presentation, accent/glow synchronisation,
-- borderless minimap styling, button gradients, and semantic typography. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 2D Component Polish"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2D_COMPONENT_POLISH"
...
if controller then
	...
	local knownPhase2D = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE2D_COMPONENT_POLISH
...
local function updateLayout()
	...
	carScroll.Position = UDim2.fromOffset(innerInset - cardStrokeSafePadding, headerHeight + cardTopSafePadding)
	carScroll.Size = UDim2.new(1, -(innerInset - cardStrokeSafePadding) * 2, 1, -(headerHeight + cardTopSafePadding + 78))
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase2D", 3000, function()
	updateRuntime()
end)
]=]
...
info("Installed the canonical isolated Phase 2D component polish.")
info("Vehicle cards now receive configurable top stroke clearance and dropdown headers toggle closed.")
info("Dropdown shells are neutral, button accent/glow states stay paired, and the minimap is borderless.")
info("Phase 2D does not patch NeoTokyoRacersClient_Bootstrap_Shadow_Disabled.")
info("Next: restart Play and verify normal PC plus Laptop device emulation.")
