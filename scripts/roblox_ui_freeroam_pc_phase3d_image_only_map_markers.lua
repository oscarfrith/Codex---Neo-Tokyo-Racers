-- Neo Tokyo Racers - PC Free-Roam UI Phase 3D Image-Only Map Markers
...
-- Phase 3C and adds image-only player/north markers with transparent containers.
-- It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 3D Image-Only Map Markers"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE3D_IMAGE_ONLY_MAP_MARKERS"
...
if controller then
	...
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H or knownPhase2I or knownPhase3A or knownPhase3B or knownPhase3C or knownPhase3D, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
info("Installed the canonical isolated Phase 3D image-only map markers.")
info("MapPlayerIcon and MapNorthArrow render with transparent backgrounds and no frames, borders, glows, or fallbacks.")
info("Player and north-arrow sizes plus north-arrow margin are editable under DesktopFreeRoamHud.Layout.")
info("Phase 3D preserves the north-up 2D map and does not patch driving, bootstrap, server, VFX, or mobile UI.")
info("Next: restart Play and verify both configured images, crop, heading, and responsive placement.")
