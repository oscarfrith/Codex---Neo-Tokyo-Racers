-- Neo Tokyo Racers - PC Free-Roam UI Phase 2H Configurable Boost Icon
...
-- Phase 2G, moves the boost group up five pixels, and replaces the BOOST text
-- with a configurable image box. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 2H Configurable Boost Icon"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2H_CONFIGURABLE_BOOST_ICON"
...
if controller then
	...
	local knownPhase2H = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F or knownPhase2G or knownPhase2H, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE2H_CONFIGURABLE_BOOST_ICON
...
local function buildMainHud()
	...
	boostIconBox = new("Frame", { Name = "BoostIconBox", BackgroundColor3 = C("PanelDeep"), BackgroundTransparency = 0.10, BorderSizePixel = 0, Size = UDim2.fromOffset(32, 32), ZIndex = 9 }, telemetry)
	corner(boostIconBox, 7)
	stroke(boostIconBox, C("ElectricBlue"), 1.3, 0.08)
	addGlow(boostIconBox, C("ElectricBlue"))
	boostIconImage = new("ImageLabel", { Name = "BoostIcon", BackgroundTransparency = 1, BorderSizePixel = 0, Image = asset("BoostIcon"), ImageColor3 = C("Text"), ScaleType = Enum.ScaleType.Fit, Position = UDim2.fromOffset(4, 4), Size = UDim2.new(1, -8, 1, -8), ZIndex = 10 }, boostIconBox)
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase2H", 3000, function(dt)
	updateRuntime(dt)
end)
]=]
...
info("Installed the canonical isolated Phase 2H configurable boost-icon repair.")
info("The boost group moved up five pixels and BOOST text is replaced by a configurable image box.")
info("Set DesktopFreeRoamHud.Assets.BoostIcon to an image asset id; size/gap remain editable in Layout.")
info("Phase 2H preserves live boost telemetry and does not patch driving, bootstrap, or mobile UI.")
info("Next: restart Play and verify icon placement plus live drain/recharge.")
