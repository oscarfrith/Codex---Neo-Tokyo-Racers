...
-- NTR_OWNED_GARAGE_ACCESS_HUD_DROPDOWNS_V1
...
function Controller.Start()
	...
	local access=Shared.ActionButton(root,{Name="Access",Text="PRIVATE",IconText="A",Size=UDim2.fromOffset(158,46),Color=UI.Colour("PanelSoft"),StrokeColor=UI.Colour("Outline")})
	local invite=Shared.ActionButton(root,{Name="Invite",Text="INVITE",IconText="+",Size=UDim2.fromOffset(158,46),Color=UI.Colour("PanelBlue"),StrokeColor=UI.Colour("Telemetry")})
	...
	local dropdown=Shared.AnchoredDropdown(root,{ZIndex=80}); local state; local busy=false; local refreshing=false; local cameraConnection
	...
	local function applyState(nextState)
		if type(nextState)~="table" or nextState.Success~=true then return false end; state=nextState; Shared.SetActionButton(access,string.upper(state.AccessMode or "PRIVATE"),nil,"A"); local count=0; for _,row in ipairs(state.InvitationRows or {}) do if row.Invited then count+=1 end end; Shared.SetActionButton(invite,count>0 and ("INVITE "..count) or "INVITE",nil,"+"); return true
	end
	...
	local function metrics() local touch=UserInputService.TouchEnabled; return {Gap=number("InteriorHudDropdownGap",5),RowHeight=number(touch and "InteriorHudTouchDropdownRowHeight" or "InteriorHudDropdownRowHeight",touch and 44 or 38),MaxRows=number(touch and "InteriorHudTouchDropdownRows" or "InteriorHudDropdownRows",touch and 4 or 5),TextSize=touch and 11 or 10,DetailTextSize=touch and 9 or 8} end
	...
	local function openAccess(force)
		if not state and not refresh(true) then return end; local rows={}; for _,mode in ipairs(state.AccessModes or {}) do table.insert(rows,{Id=mode,Text=displayMode(mode),Detail=state.AccessMode==mode and "CURRENT" or "",Selected=state.AccessMode==mode}) end
	...
	openInvites=function(force)
		if not state and not refresh(true) then return end; local rows={}; for _,item in ipairs(state.InvitationRows or {}) do table.insert(rows,{Id=item.UserId,Text=item.DisplayName,Detail=item.Invited and "REVOKE" or "INVITE",Selected=item.Invited==true,Invited=item.Invited==true}) end; if #rows==0 then table.insert(rows,{Text="NO OTHER PLAYERS",Disabled=true}) end
