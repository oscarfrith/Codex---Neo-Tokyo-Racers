-- Neo Tokyo Racers - PC Free-Roam UI Phase 2F Vehicle Card Content Offset
...
-- Phase 2D and physically moves the vehicle-card content layer below the
-- clipped scrolling-frame boundary. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 2F Vehicle Card Content Offset"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2F_VEHICLE_CARD_CONTENT_OFFSET"
...
if controller then
	...
	local knownPhase2F = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C or knownPhase2D or knownPhase2E or knownPhase2F, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
controller.Source = [=[
-- NTR_PC_FREEROAM_UI_PHASE2F_VEHICLE_CARD_CONTENT_OFFSET
...
RunService:BindToRenderStep("NTR_PCFreeRoamHudPhase2F", 3000, function()
	updateRuntime()
end)
]=]
...
info("Installed the canonical isolated Phase 2F vehicle-card content-offset repair.")
info("Vehicle cards now live in CardContent, physically offset below VehicleGrid's clip boundary.")
info("Phase 2F preserves the confirmed Phase 2D component polish and does not patch the bootstrap.")
