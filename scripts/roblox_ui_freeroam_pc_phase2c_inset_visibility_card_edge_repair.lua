...
--
-- Canonically replaces only the isolated PC free-roam HUD/controller to repair
-- GUI inset positioning, desktop/laptop eligibility, deterministic visibility,
-- and clipped vehicle-card strokes. It does not patch the register-limited
-- bootstrap or change gameplay/server/VFX/driving systems.

local PHASE = "NTR PC Free-Roam UI Phase 2C Inset Visibility Card Edge Repair"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE2C_INSET_VISIBILITY_CARD_EDGE_REPAIR"
...
if controller then
	...
	local knownPhase2C = string.find(controller.Source, MARKER, 1, true)
	assert(knownPhase1 or knownPhase2B or knownPhase2C, "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
info("Installed the canonical isolated Phase 2C inset/visibility/card-edge repair.")
info("Desktop eligibility now accepts keyboard/mouse laptops even when touch capability is reported.")
info("Major-menu suppression now requires the actual garage ScreenGui or a visible semantic menu root.")
info("Phase 2C does not patch NeoTokyoRacersClient_Bootstrap_Shadow_Disabled.")
info("Next: restart Play and verify normal PC plus Laptop device emulation without resizing.")
