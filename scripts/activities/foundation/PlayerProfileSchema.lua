...
function PlayerProfileSchema.Normalize(profile, startingCash)
	...
	-- Street Life (additive, SchemaVersion unchanged): Driver Rank and player settings.
	local progression = typeof(profile.Progression) == "table" and profile.Progression or {}
	progression.Rank = math.max(1, math.floor(tonumber(progression.Rank) or 1))
	progression.Xp = math.max(0, math.floor(tonumber(progression.Xp) or 0))
	progression.LifetimeXp = math.max(0, math.floor(tonumber(progression.LifetimeXp) or 0))
	profile.Progression = progression
	local settings = typeof(profile.Settings) == "table" and profile.Settings or {}
	local access = settings.PassengerAccess
	settings.PassengerAccess = (access == "Friends" or access == "Anyone" or access == "Nobody") and access or "Friends"
