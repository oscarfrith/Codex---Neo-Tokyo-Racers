...
local desktop=game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud")
...
local desktopLayout=game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Layout")
...
local desktopDefaults=game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Defaults")
local desktopEffects=game:GetService("ReplicatedStorage"):WaitForChild("Config"):WaitForChild("UI"):WaitForChild("DesktopFreeRoamHud"):WaitForChild("Effects")
local remotes=game:GetService("ReplicatedStorage"):WaitForChild("Remotes")
local garage=game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Garage")
...
local categories=game:GetService("ReplicatedStorage"):WaitForChild("Assets"):WaitForChild("VehiclePreviews"):WaitForChild("Categories")
...
local old=playerGui:FindFirstChild("MobileFreeRoamHud_Phase1"); if old then old:Destroy() end
local gui=new("ScreenGui",{Name="MobileFreeRoamHud_Phase1",IgnoreGuiInset=true,ResetOnSpawn=false,DisplayOrder=88,ZIndexBehavior=Enum.ZIndexBehavior.Sibling},playerGui)
...
local mapFrame=new("CanvasGroup",{Name="Minimap",BackgroundColor3=DEEP,BackgroundTransparency=.28,BorderSizePixel=0,Size=UDim2.fromOffset(170,170),Position=UDim2.fromOffset(0,0),ClipsDescendants=true,ZIndex=5},root); corner(mapFrame,9)
-- CanvasGroup clips the rotated map to the box and rounded corners; the rotator turns it around the centre (the player).
local mapRotator=new("Frame",{Name="MapRotator",AnchorPoint=Vector2.new(.5,.5),BackgroundTransparency=1,BorderSizePixel=0,Position=UDim2.fromScale(.5,.5),Size=UDim2.fromScale(1,1),ZIndex=6},mapFrame)
...
local mapPlayerMarkers=require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("FreeRoamMapPlayerMarkers")).new({
	Container=mapFrame,
...
local mapIcons=require(game:GetService("ReplicatedStorage"):WaitForChild("Modules"):WaitForChild("Game"):WaitForChild("UI"):WaitForChild("MapIconLayer")).new({
	Container=mapFrame,Surface="Minimap",ZIndex=9,Mobile=true,Font=FONT,
	Theme={PanelDeep=DEEP,Panel=PANEL,Outline=PINK,OutlineSoft=PINK_SOFT,Telemetry=CYAN,ElectricBlue=BLUE,HighSpeed=C("HighSpeed",PINK),Danger=DANGER,Text=WHITE,Muted=MUTED},
})
...
local routeRenderer=RouteGuide.newMapRenderer({Canvas=mapCanvas,Container=mapFrame,ZIndex=7,ChipZIndex=10,Font=FONT,Colours={Line=CYAN,Activity=PINK,Outline=DEEP,ChipBackground=DEEP,ChipText=WHITE}})
...
local northCornerAnchor,northCornerPosition=north.AnchorPoint,north.Position
...
-- Driver Rank strip (Street Life): just below the cash chip; attributes come from ProgressionService.
-- Sibling of the cash chip (which clips its children); follows the chip position and width.
local rankStrip=new("Frame",{Name="RankStrip",BackgroundColor3=DEEP,BackgroundTransparency=.28,BorderSizePixel=0,Size=UDim2.fromOffset(170,18),Visible=false,ZIndex=5},root); corner(rankStrip,6)
local function placeRankStrip() rankStrip.Position=cash.Position+UDim2.fromOffset(0,cash.Size.Y.Offset+4); rankStrip.Size=UDim2.fromOffset(cash.Size.X.Offset,18) end
cash:GetPropertyChangedSignal("Position"):Connect(placeRankStrip); cash:GetPropertyChangedSignal("Size"):Connect(placeRankStrip); placeRankStrip()
local rankText=label(rankStrip,"Rank","RANK 1",UDim2.new(.5,0,1,-3),UDim2.fromOffset(6,0),9,CYAN); local xpText=label(rankStrip,"Xp","",UDim2.new(.5,-6,1,-3),UDim2.new(.5,0,0,0),8,MUTED,Enum.TextXAlignment.Right)
local xpTrack=new("Frame",{Name="XpTrack",BackgroundColor3=SOFT,BorderSizePixel=0,Position=UDim2.new(0,5,1,-4),Size=UDim2.new(1,-10,0,2),ZIndex=6},rankStrip); local xpFill=new("Frame",{Name="XpFill",BackgroundColor3=CYAN,BorderSizePixel=0,Size=UDim2.fromScale(0,1),ZIndex=7},xpTrack)
local function renderRank() local rank=player:GetAttribute("Rank"); local into=tonumber(player:GetAttribute("XpIntoRank")) or 0; local need=tonumber(player:GetAttribute("XpForNext")) or 0; rankStrip.Visible=rank~=nil and player:GetAttribute("OwnedGarageInside")~=true and cash.Visible; rankText.Text="RANK "..tostring(rank or 1); xpText.Text=need>0 and (tostring(into).." / "..tostring(need).." XP") or "MAX"; xpFill.Size=UDim2.fromScale(need>0 and math.clamp(into/need,0,1) or 1,1) end
for _,attribute in ipairs({"Rank","XpIntoRank","XpForNext","OwnedGarageInside"}) do player:GetAttributeChangedSignal(attribute):Connect(renderRank) end; cash:GetPropertyChangedSignal("Visible"):Connect(renderRank); renderRank()
...
local function closeModal() shade.Visible=false; modal.Visible=false; player:SetAttribute("MobileMajorMenuOpen",false); clear(modalBody) end
shade.Activated:Connect(closeModal)
local function openModal(title,width,height) clear(modalBody); setModalReference(width,height); modalTitle.Text=title; shade.Visible=true; modal.Visible=true; player:SetAttribute("MobileMajorMenuOpen",true) end
...
local function showSettings()
	openModal("SETTINGS",tonumber(read(config,"SettingsModalWidth",720)) or 720,math.max(490,tonumber(read(config,"SettingsModalHeight",420)) or 420))
	local selected=tostring(player:GetAttribute("MobileControlMode") or read(config,"DefaultControlMode","Arrows"))
	segmented(modalBody,0,"MOBILE CONTROLS",{"Arrows","Thumbstick","Tilt"},selected,function(option) player:SetAttribute("MobileControlMode",option); showSettings() end,{Tilt=not UserInputService.GyroscopeEnabled})
	...
	local access=tostring(player:GetAttribute("PassengerAccess") or "Friends")
	segmented(modalBody,286,"PASSENGERS",{"Friends","Anyone","Nobody"},access,function(option)
		local remotes=game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):FindFirstChild("Activities"); local remote=remotes and remotes:FindFirstChild("ActivityInvoke")
		if remote then task.spawn(function() local ok,reply=pcall(function() return remote:InvokeServer("SetPassengerAccess",{Access=option}) end); if not (ok and type(reply)=="table" and reply.Ok) then showToast("PASSENGER SETTING NOT SAVED",false) end; if modal.Visible and modalTitle.Text=="SETTINGS" then showSettings() end end) end
	end)
...
-- Tapping the minimap opens the full map (FullMapUI, started by ClientBase); its legend holds the route destinations.
...
local routeButton=new("TextButton",{Name="RouteButton",Text="",AutoButtonColor=false,BackgroundTransparency=1,BorderSizePixel=0,Size=UDim2.fromScale(1,1),ZIndex=11},mapFrame)
routeButton.Activated:Connect(showFullMap)
...
local function setCarMenuOpen(open)
	carMenuOpen=open==true; closeCarChoice(); carDismiss.Visible=carMenuOpen; carPanel.Visible=carMenuOpen; player:SetAttribute("MobileFreeRoamCarMenuOpen",carMenuOpen)
...
local presentation=game:GetService("Players").LocalPlayer:WaitForChild("PlayerScripts"):WaitForChild("Runtime"):WaitForChild("UI"):FindFirstChild("FreeRoamHudPresentationMode")
...
-- FullMapOpen: FullMapUI owns the screen (and RouteGuide.Update) while the full map is open.
local function majorMenu() return player:GetAttribute("GarageSessionActive")==true or playerGui:GetAttribute("OwnedGarageManagementOpen")==true or player:GetAttribute("FullMapOpen")==true end
player:GetAttributeChangedSignal("FullMapOpen"):Connect(function() if player:GetAttribute("FullMapOpen")==true then if carMenuOpen then setCarMenuOpen(false) end closeModal() end end)
...
local displayedPos=nil; local displayedHeading=0; local displayedMapHeading=nil; local displayedBoost=1; local lastSize=Vector2.zero; local lastInside=nil
...
RunService.RenderStepped:Connect(function(dt)
	...
	if s then
		...
		if rotation.Mode=="Camera" then
			...
			mapTargetHeading=camera and mapPlayerMarkers.MapHeading(camera.CFrame.LookVector,cosine,sine,flipX,flipZ) or displayedMapHeading
		elseif rotation.Mode=="Subject" then
			mapTargetHeading=heading-tonumber(read(desktopLayout,"MapRotationOffsetDegrees",0))
		end
		...
		playerMarker.Rotation=B(desktopDefaults,"MapPlayerIconRotates",true) and (displayedHeading+mapRotation) or mapRotation
		mapPlayerMarkers.PlaceNorth(north,rotation,mapRotation,mapSize,northCornerAnchor,northCornerPosition)
		...
		mapIcons:Step(dt,{MapRotationDegrees=mapRotation,MapVisible=mapFrame.Visible and gui.Enabled,LocalWorldPosition=position,MapSize=mapSize,VisibleStuds=visible,CoordinateCosine=cosine,CoordinateSine=sine,FlipX=flipX,FlipZ=flipZ})
		RouteGuide.Update(position)
		routeRenderer:Step({MapVisible=mapFrame.Visible and gui.Enabled,FullMapStuds=fullStuds,WorldCenter=Vector2.new(tonumber(read(desktopLayout,"MapWorldCenterX",0)),tonumber(read(desktopLayout,"MapWorldCenterZ",0))),CoordinateCosine=cosine,CoordinateSine=sine,FlipX=flipX,FlipZ=flipZ,PixelScale=subpixelFactor,MapRotationDegrees=mapRotation,LocalWorldPosition=position,MapSize=mapSize,VisibleStuds=visible})
	else
