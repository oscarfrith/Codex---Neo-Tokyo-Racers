...
- Vehicle Phase AO is prepared for Studio install. It replaces the visible legacy Brakes, Converter, Fuel System, and generic upgrade controls with per-module Performance cards, next-level previews, and purchases through the confirmed Phase AN action. The stats panel gains an always-visible E-S tier/index header and contextual detailed variables.
