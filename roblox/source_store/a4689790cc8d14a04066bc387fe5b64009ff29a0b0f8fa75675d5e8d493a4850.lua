...
local RACE_VEHICLE_ATTRIBUTES = {
	...
	"RaceRunId",
	"RaceMode",
	"RaceFinishedPendingExit",
}

local function playerAllowsMarker(otherPlayer)
	return otherPlayer:GetAttribute("GarageSessionActive") ~= true
		and otherPlayer:GetAttribute("OwnedGarageInside") ~= true
		and otherPlayer:GetAttribute("RaceSessionActive") ~= true
end

local function vehicleIsInRace(vehicle)
	if not vehicle then return false end
	if vehicle:GetAttribute("RaceParticipant") == true then return true end
	if vehicle:GetAttribute("RaceRunId") ~= nil then return true end
	if vehicle:GetAttribute("RaceFinishedPendingExit") == true then return true end
	local mode = tostring(vehicle:GetAttribute("RaceMode") or "")
	return mode ~= ""
end
...
-- Shared minimap rotation rules for both HUD render owners. The map turns by
-- -heading around its centre; headings are screen degrees clockwise from up.
local ROTATION_MODES = { Camera = true, Subject = true, NorthUp = true }

function Module.ResolveRotation(config, localPlayer)
	local mode = tostring(config:GetAttribute("MapRotationMode") or "Subject")
	if not ROTATION_MODES[mode] then mode = "Subject" end
	local choice = localPlayer and localPlayer:GetAttribute("MinimapMode")
	if choice == "NORTH UP" then
		mode = "NorthUp"
	elseif choice == "ROTATE" and mode == "NorthUp" then
		mode = "Subject"
	end
	local northMode = tostring(config:GetAttribute("MapNorthArrowMode") or "Hidden")
	if northMode ~= "Orbit" and northMode ~= "Corner" and northMode ~= "Hidden" then northMode = "Hidden" end
	return {
		Mode = mode,
		Response = math.max(0, numberAttribute(config, "MapRotationResponse", 10)),
		NorthMode = northMode,
		NorthInset = math.max(0, numberAttribute(config, "MapNorthOrbitInset", 16)),
	}
end

function Module.MapHeading(lookVector, cosine, sine, flipX, flipZ)
	local x, z = lookVector.X, lookVector.Z
	if flipX then x = -x end
	if flipZ then z = -z end
	local mappedX = x * cosine - z * sine
	local mappedZ = x * sine + z * cosine
	if mappedX * mappedX + mappedZ * mappedZ < 1e-6 then return nil end
	return math.deg(math.atan2(mappedX, -mappedZ))
end

function Module.StepHeading(displayed, target, response, dt)
	if target == nil then return displayed or 0 end
	if displayed == nil then return target end
	local alpha = response <= 0 and 1 or 1 - math.exp(-response * math.max(0, dt or 1 / 60))
	return displayed + ((target - displayed + 180) % 360 - 180) * alpha
end

-- Places the north indicator: hidden, on the rim when the map rotates, or at
-- the owner's corner placement.
function Module.PlaceNorth(arrow, rotation, mapRotation, mapSize, cornerAnchor, cornerPosition)
	local visible = rotation.NorthMode ~= "Hidden"
	if arrow.Visible ~= visible then arrow.Visible = visible end
	if not visible then return end
	if arrow.Rotation ~= mapRotation then arrow.Rotation = mapRotation end
	local anchor, position
	if rotation.Mode ~= "NorthUp" and rotation.NorthMode == "Orbit" then
		local radians = math.rad(mapRotation)
		local radius = math.max(0, 0.5 - rotation.NorthInset / math.max(1, mapSize))
		anchor = Vector2.new(0.5, 0.5)
		position = UDim2.fromScale(0.5 + radius * math.sin(radians), 0.5 - radius * math.cos(radians))
	else
		anchor, position = cornerAnchor, cornerPosition
	end
	if arrow.AnchorPoint ~= anchor then arrow.AnchorPoint = anchor end
	if arrow.Position ~= position then arrow.Position = position end
end
...
function Module:Step(dt, state)
	...
	local mapRotation = math.rad(tonumber(state.MapRotationDegrees) or 0)
	local rotationCosine = math.cos(mapRotation)
	local rotationSine = math.sin(mapRotation)
	...
	for _, record in pairs(self.Records) do
		...
		local rotatedX = mappedX * rotationCosine - mappedZ * rotationSine
		local rotatedZ = mappedX * rotationSine + mappedZ * rotationCosine
		local target = Vector2.new(half + rotatedX * uiPerStud, half + rotatedZ * uiPerStud)
