...
- VFX mobile late-socket repair is generated as the next Studio test for intermittent missing engine/boost/stabiliser VFX on mobile: `scripts/roblox_vfx_mobile_late_socket_rescan_repair.lua`. It patches `VehicleVFXController` to rescan for late-replicated VFX sockets after the initial attach.
...
- Use `scripts/roblox_vfx_mobile_late_socket_rescan_repair.lua` if engine, boost, or stabiliser VFX intermittently fail to appear on mobile while the same vehicle works after reload/desktop. It is a guarded exact-source VFX module patch; refresh the Studio mirror after running and confirming it.
