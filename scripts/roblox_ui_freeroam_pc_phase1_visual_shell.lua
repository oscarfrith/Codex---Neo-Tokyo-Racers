-- Neo Tokyo Racers - PC Free-Roam UI Phase 1 Visual Shell
-- Run in Roblox Studio Command Bar while in Edit mode.
--
-- Installs an isolated PC-only free-roam HUD/controller, editable visual config,
-- and an editable dealership teleport marker. It does not patch the
-- register-limited client bootstrap and does not implement real minimap motion,
-- authoritative dealership teleporting, Developer Product receipts, persisted
-- settings, or the final boost telemetry bridge.

local PHASE = "NTR PC Free-Roam UI Phase 1 Visual Shell"
local MARKER = "NTR_PC_FREEROAM_UI_PHASE1_VISUAL_SHELL"
...
value(assets, "StringValue", "MapFeatherImage", "")
...
if controller then
	...
	assert(string.find(controller.Source, MARKER, 1, true), "Refusing to replace an unknown DesktopFreeRoamHudController_Active source.")
else
...
info("Preserved existing config values on rerun.")
info("Phase 1 does not patch NeoTokyoRacersClient_Bootstrap_Shadow_Disabled.")
info("Next: restart Play and complete the Phase 1 visual-shell verification checklist.")
